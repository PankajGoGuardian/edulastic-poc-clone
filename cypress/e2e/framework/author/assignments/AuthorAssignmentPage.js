...
class AuthorAssignmentPage {
  ...
  getAssignmentTypeByTestId = (id) =>
    this.getAssignmentRowsTestById(id).find('[data-cy="type"]')
  ...
  getSubmittedByAssignmentRow = () =>
    cy.get('@assignmentRow').find('[data-cy="submitted"]')
  ...
  clickOnDuplicateAndWait = () => {
    ...
    return cy.wait('@duplicate').then((xhr) => {
      ...
      return cy
        .saveTestDetailToDelete(xhr.response.body.result._id)
        .then(() => xhr.response.body.result._id)
    })
  }
  ...
  clickOnPreviewTestAndVerifyId = (TestID) => {
    ...
    cy.wait('@viewAsStudent').then((xhr) =>
      expect(xhr.response.body.result._id).to.equal(TestID)
    )
  }
  ...
  clickOnReleaseGrade = () =>
    cy.get('[data-cy="release-grades"]').click({ force: true })
  ...
  clickOnExpressGraderByTestId = (testId, assignmentid) => {
    ...
    if (assignmentid)
      ...
      this.getAssignmentRowsTestById(testId)
        ...
        .click({ force: true })
    if (assignmentid)
      cy.url().should('contain', `author/expressgrader/${assignmentid}`)
  ...
  clickOnStatndardBasedReportByTestId = (testId, assignmentid) => {
    ...
    if (assignmentid)
      ...
      this.getAssignmentRowsTestById(testId)
        ...
        .click({ force: true })
    if (assignmentid)
      cy.url().should('contain', `/author/standardsBasedReport/${assignmentid}`)
    else cy.url().should('contain', '/author/standardsBasedReport/')
    return cy
      .get('[class^="styled__ReportTitle"]')
      .contains('Standard performance')
  }
  ...
  clickChooseFromTestLibrary = () =>
    this.getChooseFromTestlibraryButton().click({ force: true })
  ...
  verifySubmitted = (submitted) =>
    this.getSubmitted().should('have.text', submitted)
  ...
  verifyAssignmentRowByTestId = (
    ...
    if (total)
      this.getSubmittedByAssignmentRow().should(
        'have.text',
        `${submitted} of ${total}`
      )
