...
export default class LCBHeader {
  ...
  clickOnExpressGraderTab = () => this.getExpressGraderTab().first().click()

  submitConfirmationInput = () => cy.get('[data-cy="submitConfirm"]').click()

  clickOnStandardBasedReportTab = () =>
    this.getStandardBasedReportTab().click({ force: true })
  ...
  clickOnOpenPause = (isPause) => {
    ...
    cy.wait('@togglePause').then((xhr) =>
      assert(xhr.status === 200, `verify pause-open request ${xhr.status}`)
    )
  }
  ...
  clickOnClose = (IsCloseAllowed = true, isConfirmRequired = true) => {
    ...
    cy.wait('@close').then((xhr) =>
      ...
      expect(xhr.status, `verify close request`).to.eq(
        IsCloseAllowed ? 200 : 403
      )
    )
  ...
  clickOnOpen = () => {
    ...
    return cy
      .wait('@open')
      .then((xhr) =>
        assert(xhr.status === 200, `verify open request ${xhr.status}`)
      )
  }
  ...
  clickOnMarkAsDone = () => {
    ...
    cy.wait('@markDone').then((xhr) =>
      assert(xhr.status === 200, `verify mark as done request ${xhr.status}`)
    )
  }
  ...
  verifyAssignmentStatus = (status) =>
    this.getAssignmentStatus().should('contain', status)

  verifyAssignmentDueDate = (dueDate) =>
    this.getAssignmentStatus().should(
      'contain',
      `Due on ${Cypress.moment(dueDate).format('MMM DD, YYYY')}`
    )
