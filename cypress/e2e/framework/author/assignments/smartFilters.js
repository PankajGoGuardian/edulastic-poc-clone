...
export default class SmartFilters {
  ...
  clickOnFolderByName = (folderName) =>
    cy.get(`[title="${folderName}"]`).first().click({ force: true })

  renameFolder = (currentName, newName, isValid = true) => {
    cy.server()
    cy.route('PUT', '**/user-folder/**').as('updateFolder')
    cy.get(`[data-cy="${currentName}"]`).find('[data-cy="moreButton"]').click()
    cy.get('[data-cy="rename"]').last().click({ force: true })
  ...
  verifyFolderVisible = (folderName) =>
    cy.get(`[data-cy="${folderName}"]`).should('be.visible')

  verifyFolderNotVisible = (folderName) =>
    cy.get(`[data-cy="${folderName}"]`).should('not.be.visible')
  ...
  moveToFolder = (folderName, isValid = true) => {
    ...
    cy.get('.ant-modal').contains('span', 'Move').click({ force: true })

    if (isValid) {
      cy.wait('@updateFolder').then((xhr) => expect(xhr.status).to.eq(200))
      cy.contains(`successfully moved to ${folderName} folder`).should(
        'be.visible'
      )
    } else
      cy.contains(`Test already exist in ${folderName} folder`).should(
        'be.visible'
      )
  }
