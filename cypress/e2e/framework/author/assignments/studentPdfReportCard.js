...
export default class StudentsReportCard {
  ...
  getStudentName = () =>
    this.selectedReportContainer().find('[data-cy="report-student-name"]')

  getTestName = () =>
    this.selectedReportContainer().find('[data-cy="report-test-name"]')

  getSubjects = () =>
    this.selectedReportContainer().find('[data-cy="report-subject"]')

  getClassName = () =>
    this.selectedReportContainer().find('[data-cy="report-class-name"]')

  getTestStartDate = () =>
    this.selectedReportContainer().find('[data-cy="report-test-date"]')

  getOverallFeedBack = () =>
    this.selectedReportContainer().find('[data-cy="report-feedback" ]')

  getOverallPerformance = () =>
    this.selectedReportContainer().find(
      '[data-cy="report-overall-performance"]'
    )

  getOverallPerformanceScore = () =>
    this.selectedReportContainer().find('[data-cy="report-score"]')

  getOverallPerformanceMaxScore = () =>
    this.selectedReportContainer().find('[data-cy="report-max-score"]')

  getPerformanceBand = () =>
    this.selectedReportContainer().find('[data-cy="report-performance-band"]')

  getQuestionTable = () =>
    this.selectedReportContainer().find('[data-cy="report-question-table"]')

  getStandardTable = () =>
    this.selectedReportContainer().find('[data-cy="report-standard-table"]')
  ...
  getQuestionTableHeaderElements = () =>
    this.getQuestionTableHeader().find('th')

  getStandardTableHeaderElements = () =>
    this.getStandardTableHeader().find('th')
  ...
  uncheckAllHeaderOptionsCheckBoxes = () => {
    ;[
      ..._.values(REPORT_HEADERS.QEST_TABLE.optional),
      ..._.values(REPORT_HEADERS.STAND_TABLE.optional),
    ].forEach((value) => {
      this.getCheckBoxOptionByValue(value).uncheck({ force: true })
    })
    this.getCheckBoxOptionByValue(REPORT_HEADERS.OVER_ALL_PERF).uncheck({
      force: true,
    })
  }

  checkOptionByValue = (value) =>
    this.getCheckBoxOptionByValue(value).check({ force: true })
  ...
  verifyClassName = (className) =>
    this.getClassName().should('have.text', className)

  verifyTestDate = () =>
    this.getTestStartDate().should(
      'contain.text',
      Cypress.moment(new Date()).format('MMM DD, YYYY')
    )

  verifyOverallPerfomanceScore = (score, maxScore, perf) => {
    this.getOverallPerformanceScore().should(
      'have.text',
      ` ${_.round(parseFloat(score), 2)}`
    )
    this.getOverallPerformanceMaxScore().should(
      'have.text',
      maxScore.toString()
    )
    this.getOverallPerformance().should(
      'have.text',
      `${_.round(perf.split('%')[0])}%`
    )
  }
  ...
  verifyAttemptTypeInQuestionTable = (attemptType) => {
    const {
      evaluationClass,
      evaluationColor,
    } = this.getEvaluationClassAndColor(attemptType)
  ...
  verifyStandardDomainOfselectedRow = (standard) => {
    this.getEntryByIndexOfSelectedRow(0)
      ...
      .then((txt) => {
        expect(
          standard.includes(txt),
          `expected standard domain is ${standard} and got ${txt}`
        ).to.be.true
      })
  }
  ...
  verifyQuestionTableHeaders = ({ questionHeaderOptions }) => {
    this.getQuestionTableHeaderElements().should(
      'have.length',
      questionHeaderOptions.length
    )
    questionHeaderOptions.forEach((header, index) => {
      ...
      switch (header) {
        ...
        default:
          assert.fail(
            'question table headers did not match while verifying headers'
          )
          break
      }
      this.getQuestionTableHeaderElements().eq(index).should('contain', text)
    })
    return cy.wait(1)
  }

  verifyStandardTableHeaders = ({ standardHeaderOptions }) => {
    this.getStandardTableHeaderElements().should(
      'have.length',
      standardHeaderOptions.length
    )
    standardHeaderOptions.forEach((header, index) => {
      ...
      switch (header) {
        ...
        default:
          assert.fail(
            'standard table headers did not match while verifying headers'
          )
          break
      }

      this.getStandardTableHeaderElements().eq(index).should('contain', text)
    })
    return cy.wait(1)
  }
  ...
  getQuestionTableData = ({ attempt, questionTypeMap }) => {
    ...
    Object.keys(attempt).forEach((queNo, quesIndex) => {
      ...
      let { queKey, points, attemptData, correct, choices } = questionTypeMap[
        queNo
      ]
      ...
      switch (queKey.split('.')[0]) {
        ...
        case queTypes.CHOICE_MATRIX_INLINE:
          studentResponse =
            attemptType === attemptTypes.SKIP ? skippedResponse : TEI
          correctResponse = TEI
          break
  ...
  getStandardTableData = ({ stuAttempt, questionTypeMap }) => {
    ...
    const standardBasedReport = this.standardBasedReportPage.getStandardPerformance(
      stuAttempt,
      questionTypeMap
    )
    _.keys(standardBasedReport).forEach((stand) => {
      standardTableDataByStudent[`${stand}`] =
        standardBasedReport[stand].students[`${stuAttempt[0].stuName}`]
    })
    ...
    _.keys(standardTableDataByStudent).forEach((standard) => {
      ...
      standardTableDataByStudent[
        `${standard}`
      ].masteryStatus = getMasteryStatus(perf)
    })
