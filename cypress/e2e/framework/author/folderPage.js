...
export class FolderPage {
  ...
  getFolderOptionsByName = name => this.getFolderByName(name).find('[data-cy="folder-option"]');

  getRenameFolderButton = () => this.getButtonByAttr("rename").filter((i, ele) => dom.isVisible(ele));

  getDeleteFolderButton = () => this.getButtonByAttr("delete").filter((i, ele) => dom.isVisible(ele));
  ...
  getRemoverFromFolderInDropDown = () => this.getButtonByAttr("remove-from-folder");

  getFolderInPopUpByName = name => cy.get(".ant-modal-body").find(`[data-cy="${name}"]`);

  getUpdateFolderButton = () => this.getSubmitButtonInPopUp().contains("Update Folder");

  getDeleteButtonInPopUp = () => this.getSubmitButtonInPopUp().contains("PROCEED");

  getRemoveButtonInPopUp = () => this.getSubmitButtonInPopUp().contains("Remove");

  getCancelButtonInPopUp = () => this.getButtonByAttr("cancel");

  getAddButtonInPopUp = () => this.getSubmitButtonInPopUp().contains("Add");

  getCreateNewFolderButton = () => this.getSubmitButtonInPopUp().contains("Create New Folder");
  ...
  clickCreateNewFolderBuuton = name => {
    ...
    cy.wait("@add-new-item-folder").then(({ status }) => expect(status, `adding neww item foled`).to.eq(200));
    CypressHelper.verifyAntMesssage(`${name} created successfully`);
    this.verifyFolderVisible(name);
  };

  clickOnMoreOptionsByFolderName = name => this.getFolderOptionsByName(name).click();
  ...
  clickUpdateFolder = (newName, valid = true) => {
    ...
    if (valid) {
      cy.wait("@update-folder-name").then(({ status }) =>
        expect(status, `updating folder name ${status === 200 ? "success" : "failed"}`).to.eq(200)
      );
      CypressHelper.verifyAntMesssage(`Folder name successfully updated to "${newName}"`);
    } else {
  ...
  clickDeleteFolder = folderName => {
    ...
    cy.wait("@delete-folder").then(({ status }) =>
      expect(status, `deleting folder ${status === 200 ? "success" : "failed"}`).to.eq(200)
    );
    if (folderName) {
  ...
  showActionDropDownInHeader = () =>
    this.getActionButtonInHeader()
      .should("be.visible")
      .click();

  clickAddToFolderInDropDown = () => {
    this.showActionDropDownInHeader();
    this.getAddToFolderInDropDown()
      .should("be.visible")
      .click({ force: true });
  };
  ...
  clickCancelButtonInPopUp = () => this.getCancelButtonInPopUp().click({ force: true });

  selectFolderByNameInPopUp = name => this.getFolderInPopUpByName(name).click();
  ...
  addToFolderByFolderName = folderName => {
    ...
    cy.wait("@add-item-to-folder").then(({ status }) =>
      expect(status, `adding to folder ${status === 200 ? "success" : "failed"}`).to.eq(200)
    );
  };

  removeFromFolderByFolderName = folderName => {
    cy.server();
    cy.route("PUT", "**/user-folder/*/content-delete").as("remove-item-from-folder");
    CypressHelper.removeAllAntMessages();
    ...
    cy.wait("@remove-item-from-folder").then(({ status }) =>
      expect(status, `removing item from folder ${status === 200 ? "success" : "failed"}`).to.eq(200)
    );
  };
  ...
  verifyFolderVisible = folderName => this.getFolderByName(folderName).should("be.visible");

  verifyFolderNotVisible = folderName => this.getFolderByName(folderName).should("not.be.visible");
