...
export default class ManageGroupPage extends TeacherManageClassPage {
  ...
  clickOnGroupRowByName = (groupname) =>
    this.getClassRowByName(groupname).click()

  clickOnCreateGroupButton = () => cy.get('[data-cy="createClass"]').click()

  setDescription = (description) =>
    this.getDescription().clear().type(description)
  ...
  verifyGroupRowDetails = (groupName, studentCount, assignmentCount) => {
    this.getGroupRowDetails(groupName).then(
      ({ name, students, assignments }) => {
        expect(name, 'verify groupName on manage group tab').to.eq(groupName)
        expect(students, 'verify studentcount on manage group tab').to.eq(
          `${studentCount}`
        )
        if (assignmentCount)
          expect(
            assignments,
            'verify assignment count on manage group tab'
          ).to.eq(`${assignmentCount}`)
      }
    )
  }
