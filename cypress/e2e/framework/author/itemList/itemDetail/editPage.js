...
class EditItemPage {
  ...
  clickOnSource() {
    cy.get('[data-cy="source"]').should('be.visible').click()

    return this
  }

  clickOnCancel() {
    cy.get('button').find('Cancel').should('be.visible').click()

    return this
  }

  clickOnLayout() {
    cy.get('[data-cy="layout"]').should('be.visible').click()
    return this
  }
  ...
  chooseQuestion(qGroup, qType) {
    cy.get('body').contains(qGroup).should('be.visible').click()
