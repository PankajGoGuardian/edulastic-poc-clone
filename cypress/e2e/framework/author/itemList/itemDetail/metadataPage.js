...
class MetadataPage {
  ...
  getStandardSearchOption = () =>
    cy.get('[data-cy="searchStandardSelectItem"]').parent().prev()

  getBrowseStandards = () => cy.get('button').contains('span', 'Browse')

  getAllCurrentStandardDomains = () =>
    cy.get('.ant-spin-container').find('.tlo-item-title')
  ...
  clickShowAnswer() {
    cy.constains('Show Answers').should('be.visible').click()
    return this
  }

  clickClear() {
    cy.constains('Clear').should('be.visible').click()
    return this
  }

  clickOnNewAllignmentButton = () =>
    cy.get('[data-cy="newAligment"]').click({ force: true })
  ...
  verifySelectedSubject = (subject) =>
    CypressHelper.verifySelectedOptionInDropDownByAttr('subjectSelect', subject)

  verifySelectedStandardSet = (set) =>
    CypressHelper.verifySelectedOptionInDropDownByAttr('standardSetSelect', set)

  verifySelectedGrade = (grade) =>
  ...
  verifySelectedStandards = (standard) =>
    CypressHelper.verifySelectedOptionInDropDownByAttr(
      'searchStandardSelect',
      standard,
      true
    )

  verifySelectedDok = (dok) =>
    CypressHelper.verifySelectedOptionInDropDownByAttr('dokSelect', dok)
  ...
  verifySelectedTag = (tag) =>
    CypressHelper.verifySelectedOptionInDropDownByAttr('tagsSelect', tag, true)
