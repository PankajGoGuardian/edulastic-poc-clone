...
class ClozeDropDownPage {
  ...
  // template content
  getTemplateEditor = () =>
    cy.get('[data-placeholder="[This is the template markup]"')

  getResponseContainer = (respIndex) =>
    cy.get(`[data-cy="choice-response-${respIndex}"]`)
  ...
  verifyShuffleChoices = (index, choices, shuffled = true) => {
    const arrayOfchoices = []
    cy.get("[data-cy='drop_down_select']").eq(index).click()
  ...
  setChoiceForResponseIndex = (index, choice) => {
    ...
    cy.contains(choice).should('be.visible').click()
  ...
  createQuestion = (queKey = 'default', queIndex = 0, onlyItem = true) => {
    ...
    cy.fixture('questionAuthoring').then((authoringData) => {
      ...
      if (setAns.points) {
        this.getPoints().type('{selectall}').type(setAns.points)
      }
    })
  }
}
