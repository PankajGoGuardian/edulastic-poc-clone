...
class ClozeWithTextPage {
  ...
  getResponseBoxByIndex = (index) =>
    cy.get('[class^="ClozeTextInput"]').eq(index).find('input')

  getMixAndMatchAltAnswer = () => cy.get('[data-cy="mixAndMatchAltAnswer"]')

  getIgnoreCase = () => cy.get('[data-cy="ignoreCase"]')

  getAllowSingleLetterMistake = () =>
    cy.get('[data-cy="allowSingleLetterMistake"]')
  ...
  getResponseOnPreview = (index) =>
    cy.get('[class^="AnswerBox"]').eq(index).should('be.visible')

  getShowAnsBoxOnPreview = () =>
    cy.get(`[data-cy="answerBox"]`).should('be.visible')
    ...
    cy.fixture('questionAuthoring').then((authoringData) => {
      ...
      if (quetext) {
        ...
        quetext.forEach((element) => {
          if (element === 'INPUT')
            cy.get('[data-cmd="textinput"]').click({ force: true })
