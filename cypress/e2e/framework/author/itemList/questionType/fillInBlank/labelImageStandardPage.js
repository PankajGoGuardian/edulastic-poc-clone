...
class LabelImageStandardPage {
  ...
  checkImageOnEditor() {
    return this.getQuestionEditor().get('img').should('be.visible')
  }

  // upload image

  getDropZoneImageContainer = () =>
    this.getDragDropImagePanel().find('[class^="PreviewImage"]')
  ...
  getImageInPreviewContainer = () =>
    cy.get('[data-cy="imageInpreviewContainer"]')
  ...
  checkImageAlternate(text) {
    this.getDragDropImagePanel().find('img').should('have.attr', 'alt', text)
    return this
  }
  ...
  getAllInputPanel = () =>
    this.getDragDropImagePanel().find('[data-cy="drag-input"]')
  ...
  getDashboardBorderCheck = () =>
    cy.get('[data-cy="drag-drop-image-dashboard-check"]').closest('label')
  ...
  getShowDropAreaCheck = () =>
    cy.get('[data-cy="drag-drop-image-border-check"]').closest('label')
  ...
  getAnswerBoxInPreviewByIndex = (index) =>
    cy.get('[data-cy="drag-item"]').eq(index)

  getEditChoiceByindex = (index) => this.getChoiceByIndex(index).find('p')

  clickOnKeepAspectRation = () =>
    cy.get('[data-cy="keep-aspect-ratio"]').click({ force: true })
  ...
  addNewChoice() {
    cy.get('[data-cy="add-new-ch"]').first().should('be.visible').click()
    return this
  }

  getAddedAnsByindex = (index) =>
    cy.get('[data-cy="responses-box"]').find('[data-cy="drag-item"]').eq(index)

  checkAddedAnswers = (index, text) =>
    this.getAddedAnsByindex(index).should('contain.text', text)
  ...
  switchOnAlternateAnswer = () => {
    cy.get('[data-cy="tabs"]').find('span').contains('Alternate').click()
    return this
  }
  ...
  getResponsesBoard = () =>
    cy.get('[data-cy="responses-box"]').find('[data-cy="drag-item"]')

  getResponsesBoardContainer = (Index) => cy.get(`#drop-container-${Index}`)

  getDropDownContainerInPreview = (Index) =>
    cy.get(`[data-cy="checkAnswer"]`).eq(Index)

  getPointers = () => cy.get('[data-cy="pointers"]')

  getAddAnnotationButton = () => cy.get('[data-cy="addAnnotation"]')

  getAnnotationTextArea = () =>
    cy.get('[data-cy="annotations-container"]').find('[contenteditable="true"]')
  ...
  verifyDropDownColorByIndexInPreview = (index, attempt) => {
    this.getDropDownContainerInPreview(index).should(
      'have.css',
      'background-color',
  ...
  dragAndDropResponseToBoard(toIndex) {
    this.getResponsesBox().first().customDragDrop(`#drop-container-${toIndex}`)
    return this
  }
  ...
  verifyDropContainerBorderVisible = (index, isAvailable = true) => {
    if (!isAvailable) {
      // No border
      this.getResponsesBoardContainer(index).should(
        'have.css',
        'border',
        '0px none rgb(68, 68, 68)'
      )
    } else {
      this.getResponsesBoardContainer(index).should(
        'have.css',
  ...
  dragAndDropBoardToBoard(fromIndex, toIndex) {
    cy.get(`#drop-container-${fromIndex}`).customDragDrop(
      `#drop-container-${toIndex}`
    )
    return this
  }
  ...
  getAllChoicesRes = (resIndex) =>
    cy
      .get(`[data-cy="choice-response-container_${resIndex}"]>div`)
      .then(($element) => {
  ...
  checkShuffled = (resIndex, content) => {
    ...
    return cy.get(selector).parent().children().contains('li', content)
  }
