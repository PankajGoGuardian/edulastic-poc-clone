...
class GraphingNumberLineStandardPage extends GraphingStandardPage {
  ...
  getTitleOnBoard() {
    return this.getBoard().find('div.title:not(:empty)').filter(':visible')
  }
  ...
  getPossibleResponsesOption(index) {
    return this.getPossibleResponsesContainer().find('.ql-editor').eq(index)
  }

  getMarksOnBoard() {
    return this.getBoard().find('div.mark:not(:empty)').filter(':visible')
  }
  ...
  clickOnPossibleResponsesDeleteButton(index) {
    cy.get(
      `[data-cypress="deleteButton"][data-cy="deleteprefix${index}"]`
    ).click()
    return this
  }

  clickOnPossibleResponsesAddButton() {
    this.getPossibleResponsesContainer().contains('Add').click()
    return this
  }
  ...
  selectRenderingBase(option) {
    cy.contains('div', 'Rendering base').parent().find('select').select(option)
    return this
  }

  selectToolOnToolbar(index, option) {
    const selectOp = `[data-cy="${this.segmentTools[option]}"]`
    cy.get('[data-cy="segmentTool"]').eq(index).should('be.visible').click()
  ...
  clickOnDeleteToolOnToolbar(index) {
    this.getToolbarContainer().find('.ant-select').eq(index).next().click()
    return this
  }

  clickOnSegmentsToolbarTool(index) {
    this.getSegmentsToolbar().children().eq(index).click({ force: true })
    return this
  }

  clickOnSegmentsToolbarDeleteTool() {
    this.getSegmentsToolbar().children().last().click({ force: true })
    return this
  }
