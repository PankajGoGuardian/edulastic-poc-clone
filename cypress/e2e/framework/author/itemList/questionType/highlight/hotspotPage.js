...
class HotspotPage {
  ...
  getDropZoneImageContainer = () =>
    cy.get('[data-cy="dropzone-image-container"]')
  ...
  verifyImageWidthInQuesArea = (imageWidth) => {
    this.getDrawArea().find('img').should('have.attr', 'width', imageWidth)
  }

  verifyImageWidthInAnsArea = (imageWidth) => {
    this.getHotspotMap().find('img').should('have.attr', 'width', imageWidth)
  }
  ...
  verifyImageHeightInQuesArea = (imageHeight) => {
    this.getDrawArea().find('img').should('have.attr', 'height', imageHeight)
  }

  verifyImageHeightInAnsArea = (imageHeight) => {
    this.getHotspotMap().find('img').should('have.attr', 'height', imageHeight)
  }
  ...
  clickPolygonInDrawArea = (index) => {
    this.getDrawArea().find('polygon').eq(index).click()
  }

  clickPolygonInAnswerArea = (
    index,
    select = true,
    strokeColor = queColor.HOTSPOT_SELECT
  ) => {
    if (select) {
      this.getAnswerContainer()
        .find('polygon')
  ...
  verifyNumberOfPolygonInDrawArea = (number) => {
    this.getDrawArea().find('polygon').should('have.length', number)
  }
  ...
  isImageDisplayedInQuesArea = () => {
    this.getDrawArea().find('img').should('have.attr', 'src')
  }

  isImageDisplayedInAnsArea = () => {
    this.getHotspotMap().find('img').should('have.attr', 'src')
  }

  clickAreaUndo = () => cy.get('[data-cy="area-undo"]').parent().click()

  clickAreaRedo = () => cy.get('[data-cy="area-redo"]').parent().click()

  clickAreaClear = () => cy.get('[data-cy="area-clear"]').parent().click()
  ...
  clickMultipleCheck = (check) => {
    if (check) {
      this.getMultipleCheck().click().find('input').should('be.checked')
    } else {
      this.getMultipleCheck().click().find('input').should('not.checked')
    }
  }
  ...
  getMultipleCheck = () =>
    cy.contains('span', 'Multiple responses').closest('label')
  ...
  getMaxWidth() {
    return cy.contains('Maximum width').parent().find('input')
  }
