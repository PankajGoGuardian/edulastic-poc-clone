...
class ShadingPage {
  ...
  addAlternate = () => {
    cy.get('body').contains('+ Alternative answer and score').click()
    return this
  }

  enterPoints = (points) => {
    this.getPoints().clear().type(points)
  }
  ...
  checkFontSize(fontSize) {
    this.header.preview()
    this.getStimulus().should('have.css', 'font-size').and('eq', fontSize)
    this.header.edit()
  }
  ...
  enterQuestionText = (text) => {
    this.getQuestionEditor().clear().type(text).should('have.text', text)
  }
  ...
  getCorrectAnsRowByIndex = (index) =>
    this.getCorrectAnsCellCOntainer().find('ul').eq(index)

  setCorrectAnswerByIndex = (index) => {
    this.getCorrectAnsRowByIndex(0).find('li').eq(index).click({ force: true })
  }
  ...
  selectScoringType(option) {
    ...
    cy.get(`[data-cy="scoringType"]`).click()
  ...
  getCellsRowByIndexInLayout = (index) =>
    cy.get('[data-cy="shadesView"]').eq(2).find('ul').eq(index)
  ...
  getCorrectAnsRowByIndexOnPreview(index) {
    return this.getCellContainerInPreview().find('ul').eq(index)
  }
  ...
  getMaxSelection() {
    return cy.get('body').contains('Max selection').parent().find('input')
  }
