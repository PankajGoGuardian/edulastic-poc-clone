...
class TokenhighLightPage {
  ...
  paragraph = () =>
    cy.get('[data-cy="tabs"]').eq(0).parent().next().contains('Paragraph')

  sentence = () =>
    cy.get('[data-cy="tabs"]').eq(0).parent().next().contains('Sentence')

  word = () => cy.get('[data-cy="tabs"]').eq(0).parent().next().contains('Word')
  ...
  checkFontSize(fontSize) {
    this.header.preview()
    this.getPreviewWrapper().should('have.css', 'font-size').and('eq', fontSize)
