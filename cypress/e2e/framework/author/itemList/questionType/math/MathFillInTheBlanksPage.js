...
class MathFillInTheBlanksPage extends MathFractionPage {
  ...
  getMathDropDownKeyBoard = () =>
    cy.get('[data-cy="math-keyboard-dropdown"]').first()

  getInterMediateKeyBoardInDropDown = () =>
    cy.get('[data-cy="math-keyboard-dropdown-list-1"]')

  getBasicKeyBoardInDropDown = () =>
    cy.get('[data-cy="math-keyboard-dropdown-list-0"]')

  getDeleteAnswerMethod = () =>
    this.getMathFormulaAnswers().last().find('[data-cy="delete-answer-method"]')

  getMathquillBlockId = () =>
    cy.get('[data-cy="answer-math-input-field"]').first().find('.mq-root-block')

  checkCorrectAnswerWithResponse = (
    expectedValues,
    preview,
    inputLength,
    isCorrect,
    ...
    if (Array.isArray(expectedValues)) {
      expectedValues.forEach((expectedValue, index) => {
        if (!clearedResponse) {
          this.getAnswerMathTextArea().eq(index).typeWithDelay(expectedValue)
        } else {
  ...
  allowDecimalMarksWithResponse = (
    ...
    this.checkCorrectAnswerWithResponse(
      expected,
      preview,
      inputLength,
      isCorrect
  ...
  getMathKeyboardResponse = () =>
    cy.get('[class^="KeyboardHeader__ResponseBtn"]')
      ...
      .then((ele) => {
        temp.forEach((val) => {
          if (val == 'r') {
            ...
            this.getAnswerInputMathTextarea().first().type(val, { force: true })
          }
        })
      })
  }
  ...
  setValueFill = (input, order = 1) => {
    ...
    input.forEach((val, key) => {
      cy.wait(1000)
      this.getAnswerValueMathInput().eq(key).type(val, { force: true })
    })
    // .find('textarea')
    ...
    this.getAnswerValueMathInput().then((elements) => {
      ...
      for (let i = 0; i < len; i++)
        this.getAnswerValueMathInput()
          ...
          .type('{leftarrow}{backspace}{del}'.repeat(length || 1), {
            force: true,
          })
    })
  }
