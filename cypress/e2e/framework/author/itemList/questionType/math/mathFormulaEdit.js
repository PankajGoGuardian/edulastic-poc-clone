...
class MathFormulaEdit {
  ...
  getMathKeyBoardDropdownList = (index) =>
    cy.get(`[data-cy="math-keyboard-dropdown-list-${index}"]`)
  ...
  clearAnswerValueInput = (length) => {
    this.getAnswerValueMathInput().type('{del}'.repeat(length || 1), {
      force: true,
    })
  }
  ...
  getAnswerMathInputStyleasd = () =>
    cy.get('[data-cy="answer-math-input-style"]')

  getAnswerMathTextArea = () =>
    this.getAnswerMathInputStyleasd().find('textarea')
  ...
  checkCorrectAnswer = (
    ...
    if (Array.isArray(expectedValue)) {
      expectedValue.forEach((expValue) =>
        this.getAnswerMathTextArea().typeWithDelay(expValue)
      )
    } else {
      this.getAnswerMathTextArea().typeWithDelay(expectedValue)
    }

    this.checkNoticeMessageScore(
      preview,
      isCorrect,
  ...
  getMethodSelectionDropdow = () =>
    cy.get('[data-cy="method-selection-dropdown"]')

  getMethodSelectionDropdowList = (index) =>
    cy.get(`[data-cy="method-selection-dropdown-list-${index}"]`)

  getAnswerValueMathInput = () =>
    cy.get('[data-cy="answer-math-input"]').wait(300).next().find('textarea')

  enterAnswerValueMathInput = (input) =>
    this.getAnswerValueMathInput().typeWithDelay(input)
  ...
  getAddNewThousandsSeparator = () =>
    cy.get('[data-cy="add-new-thousands-separator"]')

  getRemoveThousandsSeparator = () =>
    cy.get('[data-cy="remove-thousands-separator"]')

  getThousandsSeparatorDropdown = () =>
    cy.get('[data-cy="thousands-separator-dropdown"]')
  ...
  getThousandsSeparatorDropdownList = (index) =>
    cy.get(`[data-cy="thousands-separator-dropdown-list-${index}"]`)
  ...
  getAnswerIgnoreTrailingZeros = () =>
    cy.get('[data-cy="answer-ignore-trailing-zeros"]')

  getAnswerIgnoreCoefficientOfOne = () =>
    cy.get('[data-cy="answer-ignore-coefficient-of-one"]')

  getAnswerInverseResult = () => cy.get('[data-cy="answer-inverse-result"]')

  getAnswerTolerancecheckbox = () =>
    cy.get('[data-cy="answer-allow-tolerance"]')
  ...
  getAnswerIgnoreLeadingAndTrailingSpaces = () =>
    cy.get('[data-cy="answer-ignore-leading-and-trailing-spaces"]')

  getAnswerTreatMultipleSpacesAsOne = () =>
    cy.get('[data-cy="answer-treat-multiple-spaces-as-one"]')
  ...
  getCurrentStoreQuestion = () => {
    const storeValue = JSON.parse(window.localStorage.getItem('persist:root'))
      .question
    return JSON.parse(storeValue).entity.data
  }

  getComposeQuestionTextBox = () =>
    this.getComposeQuestionQuillComponent().find('p')

  getComposeQuestionTextBoxLink = () =>
    this.getComposeQuestionQuillComponent().find('.ql-editor p')

  getSaveLink = () => this.getComposeQuestionQuillComponent().find('.ql-action')

  getAnswerRuleDropdownByValue = (val) =>
    cy.get(`[data-cy="answer-rule-dropdown-${val}"]`)

  getAnswerRuleArgumentInput = () =>
    cy.get(`[data-cy="answer-rule-argument-input"]`)

  getAnswerRuleArgumentSelect = () =>
    cy.get(`[data-cy="answer-rule-argument-select"]`)

  getAnswerArgumentDropdownByValue = (val) =>
    cy.get(`[data-cy="answer-argument-dropdown-${val}"]`)

  getAnswerFieldDropdownListValue = (val) =>
    cy.get(`[data-cy="answer-field-dropdown-list-${val}"]`)

  getAnswerInputMathTextarea = () =>
    cy.get(`[data-cy="answer-input-math-textarea"]`)
  ...
  getUploadImageIcon = () =>
    this.getComposeQuestionQuillComponent().find('.ql-image')
    ...
    this.getMathFormulaAnswers()
      ...
      .then((element) => {
        cy.get('[mathquill-block-id]').then((elements) => {
          ...
          cy.wrap(element)
            [inputOrder]()
            .type('{del}'.repeat(length === 0 ? 1 : length + 1), {
              force: true,
            })
  ...
  mapIsFactorisedMethodFields = (fields) =>
    Object.values(fields).forEach((field) =>
      this.setIsFactorisedMethodField(field)
    )
  ...
  setArgumentInputSignDec = (selector, input, order = 0) => {
    ...
    this[selector]()[inputOrder]().check({ force: true })
    cy.get('[data-cy="answer-significant-decimal-places"]')
      .clear({ force: true })
  ...
  getVirtualKeyBoardItem = (value) =>
    this.getVirtualKeyBoard().find(`[data-cy="virtual-keyboard-${value}"]`)

  getVirtualKeyBoardResponse = () =>
    this.getVirtualKeyBoard().find('[class^="KeyboardHeader__ResponseBtn"]')
  ...
  setAnswerArgumentDropdownValue = (value) =>
    this.getAnswerRuleArgumentSelect()
      .click({ force: true })
      .then(() =>
        this.getAnswerArgumentDropdownByValue(value).click({ force: true })
      )

  checkNoticeMessageScore = (
    preview,
    isCorrect,
