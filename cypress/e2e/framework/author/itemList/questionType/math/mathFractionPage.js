...
class MathFractionPage extends MathFormulaEdit {
  ...
  addResponseBox = () => {
    this.getVirtualKeyBoard().find('span.response-embed').click()
    this.getVirtualKeyBoard().last().contains('Response').should('be.exist')
  }

  getTemplateRoot = () =>
    cy.get('[data-cy="template-container"]').next().get('.mq-root-block')
  ...
  createQuestion = (queKey = 'default', queIndex = 0, onlyItem = true) => {
    ...
    cy.fixture('questionAuthoring').then((authoringData) => {
      ...
      this.getPointsInput().type('{selectall}').type(setAns.points)
