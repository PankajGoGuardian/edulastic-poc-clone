...
class MathTextDropDown extends MathFormulaEdit {
  ...
  clickOnMathCloze = () =>
    cy.get('[class^="ClozeMathAnswer]').click({ force: true })

  clickOnAdditionalToggle = () =>
    cy.contains('Additional Options').click({ force: true })

  uncheckAllOptions = () =>
    cy.get('[class^="Widget__WidgetMethods"]').then((ele) => {
      if (Cypress.$(ele).find('input:checked').length > 0) {
        cy.wrap(ele).find('input:checked').uncheck({ force: true })
      }
    })
  ...
  checkOption = (option) =>
    cy.get(`[data-cy="${option}"]`).check({ force: true })

  // allow variable
  setAllowedVariable = (variable) =>
    cy.get('.allowed-variables').clear().type(variable)
  ...
  setDecimalSeperator = (seperator) => {
    ...
    cy.get(`[data-cy="answer-set-decimal-separator-dropdown"]`).click({
      force: true,
    })
  ...
  setThousandSeperator = (seperator) => {
    const key =
      seperator === ' ' ? 'Space' : seperator === '.' ? 'Dot' : 'Comma'
