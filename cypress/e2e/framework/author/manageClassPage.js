...
export default class TeacherManageClassPage {
  ...
  getClassRowByName = (className) =>
    cy.get('table').contains('span', className).closest('tr')
  ...
  getStudentSearchInput = () =>
    cy.get('[placeholder="Type student name or email"]')
  ...
  setName(name) {
    this.getClassName().clear().type(name)
  }
  ...
  clearSelections = (id) => {
    cy.get('@selector').then(($ele) => {
      if (
        Cypress.$($ele).find('.ant-select-selection__choice__content').length >
        0
      ) {
        cy.wrap($ele)
  ...
  selectGrade = (grade) => {
    this.getGradeSelect().as('selector').click()
  ...
  setFullName = (fullName) =>
    cy.get('[data-cy="fullName"]').clear().type(fullName)

  setPassword = (pw) => cy.get('[data-cy="password"]').clear().type(pw)

  setConfirmPassword = (pw) =>
    cy.get('[data-cy="confirmPassword"]').clear().type(pw)
  ...
  archieveClass = () => {
    ...
    cy.wait('@archieveClass').its('status').should('be.eq', 200)
  }

  gotoLastPage = () => cy.get('[title = "Next Page"]').prev().click()

  unarchiveButtonNotVisible = () =>
    this.getunArchiveButton().should('not.be.visible')
  ...
  getDateInFormate = (datetime) =>
    Cypress.moment(datetime).format('DD MMM, YYYY')
  ...
  addMultipleStudent(users, uType) {
    ...
    return this.getStudentTextArea().then(($area) => {
      ...
      users.forEach((student) =>
        cy.get('@studentlist').type(`${student}{enter}`)
      )
  ...
  searchStudentAndAdd(username) {
    ...
    this.getToEnrollStudents().contains(username).should('be.visible')

    this.clickOnAddStudentsButton(false)
    this.clickOnDone()
  }

  verifyClassRowVisibleByName = (className) =>
    this.getClassRowByName(className).should('be.visible')

  verifyNoClassRowByName = (className) =>
    cy.get('table').contains('span', className).should('not.exist')
  ...
  clickUnArchive(success = true) {
    ...
    if (success) {
      ...
      cy.wait('@unarchive').then((xhr) => {
        expect(
          xhr.status,
          'Expected to join the school before unarchiving class'
        ).to.eq(403)
        expect(xhr.responseBody.message).to.have.string(
