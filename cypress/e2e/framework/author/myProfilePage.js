...
export default class MyProfile {
  ...
  requestNewSchool = (schoolName, address, city, zip, state) => {
    ...
    cy.wait('@user').then((xhr) => {
      let userID = JSON.stringify(xhr.url).split('/')[5]
  ...
  removeSchool(schoolName, removable = true) {
    ...
    if (removable) {
      ...
      cy.wait('@remove').then((xhr) => {
        expect(xhr.status, 'Expected to archive class before removing').to.eq(
          403
        )
        expect(xhr.responseBody).to.have.string(
          'Please archive all your classes for this school to continue.'
        )
      })
    }
