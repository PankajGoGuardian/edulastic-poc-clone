...
export default class DifferentiationPage {
  ...
  getReviewTestByIndex = (index) =>
    this.getReviewWorkContainer().find(`[data-row-key="${index}"]`)

  getChallengeTestByIndex = (index) =>
    this.getChallengeWorkContainer().find(`[data-row-key="${index}"]`)
  ...
  getChallengeSlider = () =>
    this.getChallengeWorkContainer().find('.ant-slider')

  getReviewSliderHandle = () =>
    this.getReviewWorkContainer().find('.ant-slider-handle')

  getPracticeSliderHandle = () =>
    this.getPracticeWorkContainer().find('.ant-slider-handle')

  getChallengeSliderHandle = () =>
    this.getChallengeWorkContainer().find('.ant-slider-handle')
  ...
  getReviewStandardsRows = () =>
    this.getReviewWorkContainer().find('.ant-table-row-level-0')

  getPracticeStandardsRows = () =>
    this.getPracticeWorkContainer().find('.ant-table-row-level-0')

  getChallengeStandardsRows = () =>
    this.getChallengeWorkContainer().find('.ant-table-row-level-0')
      ...
      cy.get('@slider-handlers')
        ...
        .then((val) => {
          ...
          if (minvalue < currentvalue) {
            cy.get('@min-handler').type(
              '{leftarrow}'.repeat(currentvalue - minvalue)
            )
          } else if (minvalue > currentvalue) {
            cy.get('@min-handler').type(
              '{rightarrow}'.repeat(minvalue - currentvalue)
            )
          }
        })
    }
      ...
      cy.get('@slider-handlers')
        ...
        .then((val) => {
          ...
          if (maxvalue < currentvalue) {
            cy.get('@max-handler').type(
              '{leftarrow}'.repeat(currentvalue - maxvalue)
            )
          } else if (maxvalue > currentvalue) {
            cy.get('@max-handler').type(
              '{rightarrow}'.repeat(maxvalue - currentvalue)
            )
          }
        })
    }
  ...
  clickOnAddByRecommendationType = (type) => {
    ...
    cy.wait('@addRecommendation').its('status').should('be.eq', 200)
  }

  clickOnAddPracticeWork = () =>
    cy.get('[data-cy="addButton-PRACTICE"]').click()

  clickOnAddChallengeork = () =>
    cy.get('[data-cy="addButton-CHALLENGE"]').click()

  checkCheckBoxByTestName = (testName) =>
    this.getCheckBoxByTestName(testName).check({ force: true })
  ...
  verifyReviewMasteryRange = (value) => {
    this.getReviewSlider().next().should('contain', value)
  }

  verifyPracticeMasteryRange = (value) => {
    this.getPracticeSlider().next().should('contain', value)
  }

  verifyChallengeMasteryRange = (value) => {
    this.getChallengeSlider().next().should('contain', value)
  }
  ...
  verifyStandardRow = ({
    ...
    cy.get(`[data-cy="table-${type}"]`)
      ...
      .then(($ele) => {
        ...
        if (added)
          cy.get('@columns').find('input[type="checkbox"]').and('be.disabled')
        else
          cy.get('@columns')
            .find('input[type="checkbox"]')
            .should('not.be.checked')
