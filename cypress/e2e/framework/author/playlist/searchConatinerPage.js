...
export default class PlayListSearchContainer {
  ...
  getKeywordsSearchBar = () =>
    cy.get('[placeholder="Search by keywords"]').should('be.visible')
  ...
  getSharedWithMe = () =>
    cy.get('[class *= "FilterContainer"]').contains('Shared with me')
  ...
  clickOnAuthoredbyMeFolder = () =>
    cy.get('[class *= "FilterContainer"]').contains('Authored by me').click()

  clickOnEntireLibrary = () =>
    cy.get('[class *= "FilterContainer"]').contains('Entire Library').click()

  clickOnSharedWithMe = () =>
    cy.get('[class *= "FilterContainer"]').contains('Shared with me').click()

  clickOnTestFilter = () => cy.get('[data-cy="test-filter"]').click()

  typeInSearchBar = (text) => {
    this.routeTestSearch()
    this.getKeywordsSearchBar().clear().type(text)
