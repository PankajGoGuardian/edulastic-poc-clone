...
export function duplicateAndAssignTests({
  regrade_Options,
  testid,
  assignedtestids,
  versionedtestids,
...
export function studentAttemptBasedOnStatus({
  assignedtestids,
  attemptType,
  assignmentStatus,
  students,
  ...
  assignmentStatus.forEach((status, ind) => {
    if (status !== studentSide.NOT_STARTED) {
      cy.login(
        'student',
        students[`Student${ind + 1}`].email,
        students[`Student${ind + 1}`].pass
      )
        ...
        testset.forEach((id, ind) => {
          ...
          if (status === studentSide.SUBMITTED) {
            ...
            studentTestPage.clickOnNext(
              false,
              attemptType[ind] === attemptTypes.SKIP
            )
      ...
      attemptType.forEach((attempt, attempIndex) => {
        ...
        it(`> ${title}verify edited question-'${attempt}'`, () => {
          ...
          reportsPage.verifyPercentageOnTestCardByTestId(
            versionedtestids[optionsIndex][attempIndex],
            _.round(
              (data.student[status][`${option}`][`${attempt}`] / data.points) *
                100,
              2
            )
          )
          ...
          reportsPage.verifyAchievedScoreOfQueByIndex(
            0,
            data.student[status][`${option}`][`${attempt}`]
          )
  ...
  assignmentStatus.forEach((status, statusIndex) => {
    context(`> for Student${statusIndex + 1},'${status}'`, () => {
      attemptType.forEach((attempt, attemptIndex) => {
        ...
        context(
          `> verify edited question-'attempted as ${attempt}' ${title} regrade`,
          () => {
            before('> set attempt data', () => {
              ...
              if (
                ...
                if (status === studentSide.SUBMITTED)
                  ...
                  ].attempt.Q1 = attemptTypes.MANUAL_GRADE
                else
                  attemptsData[
                    assignmentStatus.indexOf(studentSide.SUBMITTED)
            ...
            before('>click assignments', () => {
              ...
              authorAssignmentPage.clickOnLCBbyTestId(
                versionedtestids[optionsIndex][attemptIndex]
              )
            })
            ...
            if (
              ...
              context(
                '> update and verify points in question centric view',
                () => {
                  it('> upadate score and verify in question centric view', () => {
                    ...
                    attemptsData[statusIndex].attempt.Q1 =
