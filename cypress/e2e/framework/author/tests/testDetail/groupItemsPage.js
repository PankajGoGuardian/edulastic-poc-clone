...
export default class GroupItemsPage {
  ...
  getAutoSelectItemForGroup = (group) =>
    cy.get(`[data-cy="autoselect-Group ${group}"]`)

  getItemContainerForGroup = (group) =>
    cy.get(`[data-cy="item-container-Group ${group}"]`)

  getCountOfItemsInGroup = (group) =>
    this.getItemContainerForGroup(group).find('span')

  getGroupContainerByGroup = (group) =>
    cy.get('.ant-collapse-item').eq(group - 1)
  ...
  uncheckAutoSelectForGroup = (group) =>
    this.getAutoSelectItemForGroup(group).uncheck()

  // question delievery
  checkDeliverAllItemForGroup = (group) =>
    cy
      .get(`[data-cy="check-deliver-all-Group ${group}"]`)
      .check({ force: true })

  checkDeliverCountForGroup = (group) =>
    cy
      .get(`[data-cy="check-deliver-bycount-Group ${group}"]`)
      .check({ force: true })

  setItemCountForDeliveryByGroup = (group, count) =>
    cy
      .get(`[data-cy="input-deliver-bycount-Group ${group}"]`)
      .type(`{selectall}${count}`)

  // filters
  setFilterForGroup = (
    group,
    { collection, standard, dok, tags, difficulty, count }
  ) => {
    if (collection)
      CypressHelper.selectDropDownByAttribute('collection', collection)
    ...
    if (difficulty)
      CypressHelper.selectDropDownByAttribute('difficulty', difficulty)
  ...
  clickSelectItemsForGroupByNo = (group) => {
    ...
    this.getItemContainerForGroup(group).next().click()
  ...
  clickOnSaveByGroup = (group, dynamicGroup = false, deliveryCount = 1) => {
    ...
    if (dynamicGroup)
      return cy.wait('@waitForItems').then((xhr) => {
        ...
        expect(xhr.response.body.result.items.length).to.be.at.least(
          deliveryCount
        )
  ...
  clickOnCollectionByGroup = (group) =>
    cy.get(`[data-cy="collection-Group ${group}"]`).click()
  ...
  clickBrowseOnStandardsByGroup = (group) =>
    cy.get(`[data-cy="standard-Group ${group}"]`).click()

  // ======Below methods are for standard selection in pop-up of autoselect item group

  checkstandardsByName = (standards) =>
    cy.get(`[data-cy="${standards}"]`).check({ force: true })

  selectSubject = (subject) =>
    CypressHelper.selectDropDownByAttribute('subject-Select', subject)
  ...
  selectStandardsBySubGradeStandardSet = (
    subject,
    Grade,
    standardSet,
    standards
  ...
  verifyPresentItemInContainer = (group, item) =>
    this.getItemContainerForGroup(group).should(
      'contain',
      CypressHelper.getShortId(item)
    )
  ...
  verifyNoOfGroups = (count) =>
    cy.get('[data-cy^=item-container]').should('have.length', count)
  ...
  createDynamicTest = (group = 1, filterForAutoselect, overRide = false) => {
    ...
    this.selectStandardsBySubGradeStandardSet(
      subject,
      grade,
      standardSet,
      standardsToSelect
    ...
    return cy
      .wait(1)
      .then(() => this.clickOnSaveByGroup(group, true, deliveryCount))
  }

  assertNoItemsFoundWarning = () =>
    CypressHelper.verifyAntMesssage(
      'No test items found for current combination of filters.'
    )
  ...
  getItemDeliverySeq = (deliveredItemGroups, groups, shuffle = false) => {
    ...
    let deliveredArray = []
    const deliverTypeAtStudentSide = []
    ...
    deliveredItemGroups.forEach((gArray, ind) => {
      ...
      deliveredArray = [
        ...deliveredArray,
        ...deliveredGroup[Cypress._.keys(deliveredGroup)[ind]],
      ]
    })
      ...
      switch (type) {
        case deliverType.ALL:
          if (!shuffle)
            ...
            expect(deliveredGroup[ind + 1].length).to.eq(
              groups[ind + 1].deliveryCount
            )
          }
          break
