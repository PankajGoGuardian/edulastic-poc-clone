...
export default class TestAssignPage {
  ...
  getMaxAttempt = () =>
    cy.get('[inputfeatures="maxAttemptAllowed"]').find('input')

  getShuffleQue = () =>
    cy
      .get('[inputfeatures="assessmentSuperPowersShuffleQuestions"]')
      .find('button')

  getShuffleChoices = () =>
    cy
      .get('[inputfeatures="assessmentSuperPowersShuffleAnswerChoice"]')
      .find('button')

  getAnswerOnPaper = () =>
    cy
      .get('[inputfeatures="assessmentSuperPowersAnswerOnPaper"]')
      .find('button')
  ...
  getAssignmentTimeSettingInfo = () =>
    cy.get('[inputfeatures="assessmentSuperPowersTimedTest"]').find('svg')

  getAllowExit = () => cy.get('[data-cy="exit-allowed"]')

  getNavigateTolcbButtonInSuccessPage = () =>
    cy.get('[data-cy="assignButton"]').contains('Go to Live Classboard')

  getNavigateToTestLibraryPage = () =>
    cy.get('[data-cy="assignButton"]').contains('Return to TEST LIBRARY')

  getSafeExamBrowserSwitch = () =>
    cy
      .get('[inputfeatures="assessmentSuperPowersRequireSafeExamBrowser"]')
      .find('button')
  ...
  setMaxAttempt = (attempt) =>
    this.getMaxAttempt().type(`{selectall}${attempt}`)

  clickOnTypesOfReleaseScores = () =>
    cy.get('[data-cy="selectRelaseScore"]').click()
  ...
  setCheckAnsTries = (tries) =>
    cy.get('[placeholder="Number of tries"]').type(`{selectall}${tries}`)
  ...
  showOverRideSetting = () => {
    if (
      Cypress.$('[inputfeatures="assessmentSuperPowersMarkAsDone"]').length ===
      0
    ) {
  ...
  disableAllowExit = () =>
    this.getTimeSettingSwitch().then(($ele) => {
      expect($ele, 'Time switch should be enabled first').to.have.class(
        'ant-switch-checked'
      )
      this.getAllowExit()
        .parent()
  ...
  enableAllowExit = () =>
    this.getTimeSettingSwitch().then(($ele) => {
      ...
      this.getAllowExit()
        ...
        .then(($el) => {
          if (!$el.hasClass('ant-checkbox-checked'))
            this.getAllowExit().check({ force: true }).should('be.checked')
        })
    })
  ...
  showAdvancedSettings = () =>
    cy.get('[data-cy="advanced-option"]').then(($ele) => {
      if (Cypress.$($ele).text() === 'SHOW ADVANCED OPTIONS')
        cy.wrap($ele).click({ force: true })
    })
  ...
  verifyTimeAssignedForTest = (questionCount) =>
    this.getTimeSettingTextBox().should('have.value', `${questionCount}`)
  ...
  verifySelectedPerformanceBand = (band) =>
    this.getPerformanceBandDropDown().should('contain.text', band)
