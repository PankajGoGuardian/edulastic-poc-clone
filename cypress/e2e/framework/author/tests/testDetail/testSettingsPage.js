...
export default class TestSettings {
  ...
  getCheckAnswer = () =>
    cy.get('#check-answer-tries-per-question').find('input')
  ...
  getSafeExamBrowserSwitch = () =>
    cy.get('#require-safe-exame-browser').find('button')

  getQuitPassord = () => cy.get('[placeholder="Quit Password"]')

  getMarkAsDoneAutomatically = () =>
    cy.get('#mark-as-done').find('[value="automatically"]')
  ...
  selectDontReleaseScoreResponse = () =>
    cy.get('[value="DONT_RELEASE"]').click()

  setRealeasePolicy = (policy) => cy.get(`[VALUE=${policy}]`).click()

  setMaxAttempt = (attempts) =>
    cy.get('[id="maximum-attempts-allowed"]').find('input').type(attempts)

  setCheckAnswer = (checkAns) =>
    this.getCheckAnswer().type(`{selectall}${checkAns}`)

  setMaxAttempt = (maxAttempt) =>
    this.getMaxAttempt().type(`{selectall}${maxAttempt}`)
  ...
  disableAllowPause = () =>
    this.getTimeSettingSwitch().then(($ele) => {
      expect($ele, 'Time switch should be enabled first').to.not.have.class(
        'ant-switch-checked'
      )
