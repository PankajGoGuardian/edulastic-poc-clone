...
class SignupPage {
  ...
  clickOnStudent = () =>
    cy.get('[data-cy=student]').should('be.visible').click()

  clickOnTeacher = () =>
    cy.get('[data-cy=teacher]').should('be.visible').click()

  clickOnAdmin = () => cy.get('[data-cy=admin]').should('be.visible').click()

  setCode = (code) => cy.get(`[data-cy=classCode]`).clear().type(code)

  setName = (name) => cy.get(`[data-cy=name]`).clear().type(name)

  setEmail = (email) => cy.get(`[data-cy=email]`).clear().type(email)
  ...
  selectGrade = (grade) =>
    CypressHelper.selectMultipleSelectionDropDown('grade', grade)

  selectSubject = (subject) =>
    CypressHelper.selectMultipleSelectionDropDown('subject', subject)

  selectStandardSet = (standard) =>
    CypressHelper.selectMultipleSelectionDropDown('standardSet', standard)
  ...
  clickOnReqNewSchool = () => {
    cy.get(`.ant-modal-footer`).find('[data-cy="reqNewSchoolBtn"]').click()
    cy.wait('@user').then((xhr) => expect(xhr.status).to.eq(200))
  }

  clickOnRequestNewSchoolLink = () =>
    cy.contains('REQUEST A NEW SCHOOL').click()

  setSchoolName = (school) => cy.get('[data-cy="school"]').clear().type(school)

  setaddress = (address) => cy.get('[data-cy="address"]').clear().type(address)

  setcity = (city) => cy.get('[data-cy="city"]').clear().type(city)

  setzip = (zip) => cy.get('[data-cy="zip"]').clear().type(zip)
  ...
  verifyDistrict = (dist) =>
    cy.get('[data-cy="districtName"]').should('contain.text', dist)
