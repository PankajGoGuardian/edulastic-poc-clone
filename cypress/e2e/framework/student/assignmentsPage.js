...
class AssignmentsPage {
  ...
  getclass(classname) {
    ...
    cy.contains(classname).should('be.visible').click()
  }
  ...
  getTestNameById = (id) =>
    this.getAssignmentByTestId(id).find('[data-cy="testTitle"]')

  getReviewButtonById = (id) =>
    this.getAssignmentByTestId(id).find('[data-cy="reviewButton"]')
  ...
  getOkOnPopUp = () => cy.get('.ant-modal-confirm-btns').find('button').last()
  ...
  getTestTypeByTestId = (id) =>
    this.getAssignmentByTestId(id).find('[data-cy="testType"]')

  getSEBiconByTestId = (id) =>
    this.getAssignmentByTestId(id).find(
      '[title="This test requires Safe Exam Browser"]'
    )
  ...
  clickOnAssignmentButton() {
    ...
    this.getAssignmentButton()
      ...
      .click({ force: true })
      .then(() => {
        if (Cypress.$('[data-cy="attemptsCount"]').length === 1)
          this.getLaunchRetakeButton().click({ force: true })
      })
    ...
    return cy.wait(`@gettest-${attempt_index}`).then(() => {
      cy.get('[data-cy="next"]', { timeout: 30000 }).then(
        () => new StudentTestPage()
      )
    })
  }
  ...
  enterPassword = (pass) =>
    cy.get('[placeholder="Enter assignment password"]').type(pass)
  ...
  verifyTestNameById = (testName, id) =>
    this.getTestNameById(id).should('contain', testName)
  ...
  verifyPresenceOfTest = (id) =>
    cy
      .get('body', { timeout: 30000 })
      .should('have.descendants', `[data-cy="test-${id}"]`)
  ...
  verifyAssignmentIslocked = () =>
    cy.get('[data-cy="lockAssignment"]').should('exist')

  verifyAssignmentIsPaused = () =>
    cy.get('[data-cy="lockAssignment"]').contains('Paused').should('exist')

  verifyAssignmentIsOpen = (testID) =>
    this.getAssignmentButtonByTestId(testID).should('exist')

  verifyTimeAvalableForTestById = (testid, time) =>
    this.getTimeAvailableForAssignmentById(testid).should(
      'contain.text',
      `${time} minutes`
    )

  verifyAssignTypeByTestId = (id, aType) =>
    this.getTestTypeByTestId(id).should('have.text', aType)
  ...
  verifyStatusOfAssignment = (status) => {
    ...
    cy.wait(1).then(() =>
      status.forEach((sta) => expect(sta).to.be.oneOf(statuses))
    )
  }
