...
class StudenPlaylist extends PlayListReview {
  ...
  getPlaylistGradeOnCardById = () =>
    this.getPlaylistCardById(id).find('[data-cy="grade"]')

  getPlaylistSubOnCardById = () =>
    this.getPlaylistCardById(id).find('[data-cy="subject"]')

  getStartPractice = (mod, test) =>
    this.getTestByTestByModule(mod, test).find(`[data-cy="START PRACTICE"]`)
  ...
  clickOnResumeByTestByMod = (mod, test) =>
    this.clickOnPractiseResumeByTestcard(mod, test)
