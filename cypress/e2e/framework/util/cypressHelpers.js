export default class CypressHelper {
  ...
  static selectMultipleSelectionDropDown = (attribute, option) => {
    ...
    cy.get(`[data-cy=${attribute}]`).find('.ant-select-arrow').click()

    cy.wait(300) // allow list to collapse
  }

  static verifySelectedDropDownByAttribute = (
    dataCYAttributeValue,
    selectedItem
  ) => {
  ...
  static verifySelectedOptionInDropDownByAttr = (
    attr,
    value,
    isMultipleSelectionAllowed = false
  ) => {
    const clas = isMultipleSelectionAllowed
    ...
    return cy
      ...
      .then(($ele) =>
        cy
          .wrap($ele.filter((i, ele) => Cypress.$(ele).text() === value))
