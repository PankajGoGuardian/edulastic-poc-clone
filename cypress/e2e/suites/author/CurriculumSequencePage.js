...
describe(`${FileHelper.getSpecName(
  Cypress.spec.name
)} >> Check Curriculum Sequence Page`, () => {
  beforeEach(() => {
    cy.login()
  ...
  it(' > Should add a new unit', () => {
    ...
    cy.get('[data-cy=openAddUnit]').eq(0).click()

    cy.get('[data-cy=addNewUnitInputName]').type(
      'Module Cypress Unit: Test Unit'
    )
  ...
  it(' > Should assign an item', () => {
    ...
    cy.get('[data-cy=curriculumModuleRow]').then((el) => {
      ...
      cy.get('[data-cy=openAddContent]').eq(0).click()

      // expand content unit
      cy.get('[data-cy=expandCollapseContentUnit]').eq(0).click()

      // add content item to curriculum sequence first module
      cy.get('[data-cy=assignContentIcon]').eq(0).trigger('mouseover')

      cy.get('[data-cy=addContentMenuItem]').eq(1).click()
      ...
      cy.get('[data-cy=selectClass]').click().get('[data-cy=class]').click()

      // Click apply button on modal
      cy.get('[data-cy=apply]').click()

      cy.get('[data-cy=curriculumModuleRow]').eq(1).contains('ASSIGNED')
    })
  })
  ...
  it(' > Should assign all items of the module (ones that have testIds)', () => {
    ...
    // Expand and assign
    cy.get('[data-cy=assignContentIcon]').eq(1).trigger('mouseover')

    cy.get('[data-cy=addContentMenuItem]').eq(1).click()

    // Assign whole module
    cy.get('[data-cy=AssignWholeModule]').eq(0).click()
  ...
  it(' > Existing item should not be added to the module', () => {
    ...
    cy.get('[data-cy=addContentMenuItem]').eq(0).click()
            ...
            if (numberOfItemsAfterAdd !== initialTotalAssigned + 1) {
              ...
              cy.get('[data-cy=curriculumModuleRow]')
                ...
                .then((totalAssignedEl) => {
                  const numberOfItemsAfterRemove = Number(
                    totalAssignedEl.text()
                  )
                  expect(numberOfItemsAfterRemove, 'After remove').to.equal(
                    initialTotalAssigned
                  )
                })
            } else {
              expect(
                numberOfItemsAfterAdd,
                'After trying to add duplicated item'
              ).to.equal(initialTotalAssigned + 1)
  ...
  it(' > Should assign an item NOW (add to misc)', () => {
    ...
    cy.get('[data-cy=curriculumModuleRow]').then((el) => {
      ...
      // cy.debug();
      if (
        Cypress.$(el).text() === 'Misc' &&
        Cypress.$(el).find('> [data-cy=removeUnit]').length === 1
      ) {
