...
describe(`${FileHelper.getSpecName(Cypress.spec.name)}Verify Actions Button In Author Side Assignments Page`, () => {
  ...
  before("> get data of test and its itemns", () => {
    cy.deleteAllAssignments(Student1.email, Teacher.email, Teacher.pass, [daTestId]);
  ...
  context("> verify assignment assigned by DA", () => {
    it("> verify author assignents page", () => {
      testLibraryPage.sidebar.clickOnAssignment();
      authorAssignmentPage.verifyAssignmentRowByTestId(daTestId, undefined, undefined, undefined, "C");
      ...
      dropdownOptions.slice(dropdownOptions.length - 2).forEach(option => {
        authorAssignmentPage.getOptionInDropDownByAttribute(option).should("not.exist", `${option} in dropdown`);
      });
    });
