...
describe(`${FileHelper.getSpecName(Cypress.spec.name)} >> Smart Filters`, () => {
  ...
  context(`assignment folders`, () => {
    ...
    it(`move assignment to folder`, () => {
      ...
      CypressHelper.verifyAntMesssage(`${testName} successfully moved to "${folders[2]}"`);
    ...
    it(`move assignment to already moved folder`, () => {
      ...
      CypressHelper.verifyAntMesssage(`${testName} already exist in ${folders[2]} folder`);
    });
    ...
    it(`delete used folder`, () => {
      ...
      cy.contains(
        `${
          folders[2]
        } will get deleted but all tests will remain untouched. The tests can still be accessed from All Assignments.`
      ).should("be.visible");
