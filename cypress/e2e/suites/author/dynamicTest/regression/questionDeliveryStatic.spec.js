...
describe(`${FileHelper.getSpecName(Cypress.spec.name)} >> item groups`, () => {
  ...
  context('> dynamic tests having,', () => {
      ...
      it('> login as teacher verify item sequence for each student in lcb', () => {
        ...
        lcb.clickonQuestionsTab().then(() => {
          ...
          CypressHelper.getDropDownList().then((questions) => {
            expect(
              questions,
              `Expected no of questions is ${itemIds.length}`
            ).to.have.lengthOf(itemIds.length)
          })
    ...
    context(
      '> two static groups with, deliver all from group1 + deliver by count from group2',
      () => {
        before('> login as CE', () => {
          testID = undefined
          groups = {
        ...
        it("> create test having two static groups with 'deliver all' and 'deliver by count'", () => {
          ...
          groupItemsPage.setItemCountForDeliveryByGroup(
            2,
    ...
    context(
      '> one static group with deliver all and shuffle(test settings)',
      () => {
        before('> login as CE', () => {
          testID = undefined
          groups = { 1: {} }
