...
describe(`${FileHelper.getSpecName(
    ...
    Cypress._.keys(manualGradeItems).forEach((questNo) => {
      ...
      students.forEach((student, studentindex) => {
        ...
        lcb.questionResponsePage
          .getQuestionContainerByStudent(student.name)
          .as('studentQuesCard')
  ...
  context('> with all check box checked(defualt)', () => {
    before('> set header options', () => {
      ...
      reportOptions.question = Cypress._.values(
        REPORT_HEADERS.QEST_TABLE.optional
      )
      reportOptions.standard = Cypress._.values(
        REPORT_HEADERS.STAND_TABLE.optional
      )
    })
  ...
  context('> with performance band unchecked', () => {
    ...
    before('> login as teacher and generate report', () => {
      ...
      ;[...reportOptions.question, ...reportOptions.standard].forEach(
        (value) => {
          studentReportsCardPage.checkOptionByValue(value)
        }
      )
      if (reportOptions.overAllPerformanceBand)
        studentReportsCardPage.checkOptionByValue(REPORT_HEADERS.OVER_ALL_PERF)
      studentReportsCardPage.clickReportGeanerateButton()
    })

    reportVerificationFunction({
      attemptData: attemptData.slice(0, 1),
      testDetails,
      reportOptions,
      questionTypeMap,
    })
  ...
  context('> with standard performance unchecked', () => {
    before('> set header options', () => {
      reportOptions.overAllPerformanceBand = true
      reportOptions.standard = [
        REPORT_HEADERS.STAND_TABLE.optional.masteryStatus,
      ]
      reportOptions.question = Cypress._.values(
        REPORT_HEADERS.QEST_TABLE.optional
      )
  ...
  context('>redirect', () => {
    ...
    before('> manual grading', () => {
      ...
      Cypress._.keys(manualGradeItems).forEach((questNo, ind) => {
        ...
        lcb.questionResponsePage
          .getQuestionContainerByStudent(students[1].name)
          .as('studentQuesCard')
        lcb.questionResponsePage.updateScoreAndFeedbackForStudent(
          students[1].name,
          score
    ...
    reportVerificationFunction({
      attemptData: attemptData.slice(1, 2),
      testDetails,
      reportOptions,
      questionTypeMap,
    })
