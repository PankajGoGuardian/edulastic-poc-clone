...
const user = {
  email: 'teacher1.smoke.automation@snapwiz.com',
  password: 'automation',
}
...
describe(`${FileHelper.getSpecName(Cypress.spec.name)} >> Manage Class`, () => {
  ...
  context('> create new class', () => {
      ...
      manageClass
        ...
        .then(() => {
          CypressHelper.getDropDownList().then((list) =>
            expect(list).to.eql(posibleGrades)
          )
        })
    })
    ...
    it('> verify subject dropdown lists', () => {
      ...
      manageClass
        ...
        .then(() => {
          CypressHelper.getDropDownList().then((list) =>
            expect(list).to.eql(posibleSubjects)
          )
        })
    })
          ...
          manageClass
            ...
            .then(() => {
              CypressHelper.getDropDownList().then((list) =>
                expect(
                  list,
                  `verify standards${JSON.stringify(
                    list
                  )},are as per subject ${sub}`
                ).to.include.members([subjectStdMap[sub]])
              )
            })
            .wait(500)
            .focused()
  ...
  context('> add students', () => {
    ...
    it('> add one student', () => {
      ...
      manageClass.clickOnAddUserButton().then(() => {
        ...
        // cy.wait("@mygroups");
        manageClass
          .getClassRowDetails(testData.create.className)
          .then((cls) => {
  ...
  context('> edit class', () => {
    ...
    it('> edit existing class', () => {
      const randomGrade =
        posibleGrades[_.floor(Math.random() * posibleGrades.length)]
