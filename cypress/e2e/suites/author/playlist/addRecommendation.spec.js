...
} = require('../../../../../fixtures/testAuthoring')
...
describe(`${FileHelper.getSpecName(
  ...
  const assignment1 =
    'Test One for Automation - Playlist Recommendation (5.G.A.1)'
  const assignment2 =
    'Test Two for Automation - Playlist Recommendation (5.MD.A.1)'
  const assignment3 =
    'Test Three for Automation - Playlist Recommendation (5.G.A.1 and 5.G.A.2)'
  ...
  const standardMapping = [
    {
      ...
      reviewStandards: ['4.G.A.1'],
      challengeStandards: ['6.G.A.1'],
    },
    {
  ...
  context.skip('>add review recommendation and verify', () => {
    ...
    it('add review recommendation and verify teachers side', () => {
      ...
      differentiationPage.clickOnAddByRecommendationType(
    ...
    it('verify recommendation at student', () => {
      ...
      studentRecommendationTab
        .clickOnPracticeByAssignmentName(assignment1)
        .then((reviewTestItems) => {
          reviewTestItems.forEach((itemsArray) => {
            itemsArray.forEach((itmId) => {
              expect(itmId).to.be.oneOf(
                testItemsInStandard.standardTestItemsMapping[0].testItemIds
              )
            })
          })
        })
  ...
  context.skip('>add challenge recommendation and verify', () => {
    ...
    it('verify recommendation at student', () => {
      ...
      studentRecommendationTab
        .clickOnPracticeByAssignmentName(assignment1)
        .then((practiceTestItems) => {
          practiceTestItems.forEach((itemsArray) => {
            itemsArray.forEach((itmId) => {
              expect(itmId).to.be.oneOf(
                testItemsInStandard.standardTestItemsMapping[2].testItemIds
              )
            })
          })
        })
  ...
  context.skip('>add practice recommendation and verify', () => {
    ...
    it('verify recommendation at student', () => {
      ...
      studentRecommendationTab
        .clickOnPracticeByAssignmentName(assignment1)
        .then((challengeTestItems) => {
          challengeTestItems.forEach((itemsArray) => {
            itemsArray.forEach((itmId) => {
              expect(itmId).to.be.oneOf(
                testItemsInStandard.standardTestItemsMapping[1].testItemIds
              )
            })
          })
        })
  ...
  context.skip('>add test from manage content to review work', () => {
    ...
    const attempt1 = {
      studentAttempt: { Q1: 'right', Q2: 'right' },
      score: '4/4',
      mastery: '100',
    }
    const attempt2 = {
      studentAttempt: { Q1: 'wrong', Q2: 'right' },
      score: '2/4',
      mastery: '50',
    }
    ...
    it('search test and add to review work', () => {
      ...
      differentiationPage.dragTestFromSearchToSectionAndVerify(
        recommendationType.REVIEW,
        testId,
        testName
      )
    ...
    it('attempt and verify recommendation at student', () => {
      ...
      Object.keys(attempt1.studentAttempt).forEach((queNum, i) => {
        studentTestPage.attemptQuestion(
          itemKeys[i].split('.')[0],
          attempt1.studentAttempt[queNum],
          attemptData
        )
    ...
    it('verify 2nd attempt of student practice', () => {
      ...
      Object.keys(attempt2.studentAttempt).forEach((queNum, i) => {
        studentTestPage.attemptQuestion(
          itemKeys[i].split('.')[0],
          attempt2.studentAttempt[queNum],
          attemptData
        )
        studentTestPage.clickOnNext()
      })
