...
describe(`${FileHelper.getSpecName(
  ...
  context.skip('>  authoring', () => {
    ...
    it(">author playlist-'create modules'", () => {
      ...
      for (let i = 0; i < tests.length; i++) {
        ...
        playListLibrary.addTestTab.setModuleGroupNameByModule(
          i + 1,
          `module-group-${i + 1}`
        )
    ...
    it(">verify review- 'module name and test count'", () => {
      ...
      tests.forEach((id, index) => {
        ...
        playListLibrary.reviewTab
          .getModuleNameByModule(index + 1)
          .should('contain', `module-${index + 1}`)
    ...
    it(">verify review- 'standards'", () => {
      tests.forEach((id, index) => {
        playListLibrary.reviewTab.verifyStandardsByTestByModule(
          index + 1,
          1,
          standard
        )
  ...
  context.skip('>bulk actions', () => {
    before('create playlist', () => {
      cy.login('teacher', teacher.email, teacher.pass)
      playListLibrary
        .createPlayList(playListData, testIds.length / 2)
        .then((id) => {
          playlistId = id
        })
    })
    it('>bulk addition and verify review', () => {
      playListLibrary.searchFilter.clearAll()
      playListLibrary.addTestTab.bulkAddByModule(
        testIds.slice(0, testIds.length / 2),
        1
      )
      playListLibrary.header.clickOnSave()
      playListLibrary.addTestTab.bulkAddByModule(
        testIds.slice(testIds.length / 2),
        2
      )
      playListLibrary.header.clickOnSave()
