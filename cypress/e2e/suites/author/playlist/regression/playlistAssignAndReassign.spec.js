...
describe(`${FileHelper.getSpecName(Cypress.spec.name)}> playlist assigning`, () => {
  ...
  before(">create play list", () => {
    ...
    playlistlibraryPage.createPlayListWithTests({ metadata, moduledata }).then(id => {
      playListId = id;
    });
  });
  context("> play list assign", () => {
    context("> assign whole module", () => {
      ...
      it("> teacher side verification 'favourite playlist page'", () => {
        const rowData = { status: teacherSide.IN_PROGRESS, submitted: "0 of 1", graded: "0", className: classes[0] };
        ...
        [1, 2].forEach(test => {
          playlistlibraryPage.reviewTab.clickShowAssignmentByTestByModule(1, test);
          playlistlibraryPage.reviewTab.verifyAssignmentRowByTestByMod(1, test, { ...commonData, ...rowData });
        });
      });
      ...
      it("> verify 'assignments rows' in 'favourite playlist page'", () => {
        const rowData = { status: teacherSide.DONE, submitted: "1 of 1", graded: "1", className: classes[0] };
        testlibraryPage.sidebar.clickOnRecentUsedPlayList();
        testIds.forEach((test, i) => {
          playlistlibraryPage.reviewTab.clickShowAssignmentByTestByModule(1, i + 1);
          playlistlibraryPage.reviewTab.verifyAssignmentRowByTestByMod(1, i + 1, { ...commonData, ...rowData });
        });
      });
    });
    ...
    context("> one test in a module", () => {
      ...
      it("> teacher side verification 'favourite playlist page'", () => {
        ...
        playlistlibraryPage.reviewTab.verifyAssignmentRowByTestByMod(1, 1, { ...commonData, ...rowData });
      });
      ...
      it("> verify lcb,express grader and reports navigations from 'favourite playlist page'", () => {
        ...
        for (let k = 0; k < 3; k++) {
          ...
          k === 0
            ? lcb.verifyClassAndAssignmntId(class1_Id, assignMentIdObj[testIds[0]])
            : k === 1
            ? egp.verifyClassAndAssignmntId(class1_Id, assignMentIdObj[testIds[0]])
            : k === 2
            ? sbr.verifyClassAndAssignmntId(class1_Id, assignMentIdObj[testIds[0]])
            : assert.fail();
          cy.go("back");
          cy.wait("@go-to-playList");
        }
      ...
      it("> mark student as 'submitted' and 'close' test", () => {
        ...
        playlistlibraryPage.sidebar.getFavouritePlaylist();
        testlibraryPage.sidebar.clickOnAssignment();
        authorAssignmentPage.clickOnLCBbyTestId(testIds[0], assignMentIdObj[testIds[0]]);
    ...
    context("> one test in a module 'assign same test to two classes'", () => {
      ...
      it("> teacher side verification 'assignments page'", () => {
        ...
        authorAssignmentPage.getStatus().should("have.length", 2);
        authorAssignmentPage.getAssignmentRowsTestById(testIds[0]).should("have.length", 2);
      });
      ...
      it("> teacher side verification 'favourite playlist page'", () => {
        ...
        classes.forEach((clas, ind) => {
          const rowData = { status: teacherSide.IN_PROGRESS, submitted: "0 of 1", graded: "0", className: clas };
          playlistlibraryPage.reviewTab.verifyAssignmentRowByTestByMod(1, 1, { ...commonData, ...rowData }, ind);
        });
      });
      ...
      it("> verify lcb,express grader and reports navigation from favourite playlist page", () => {
        ...
        classes.forEach((clas, ind) => {
          ...
          for (let k = 0; k < 3; k++) {
            playlistlibraryPage.reviewTab.clickExpandByModule(1);
            playlistlibraryPage.reviewTab.clickLcbIconByTestByIndex(1, 1, k, ind);
            k === 0
              ? lcb.verifyClassAndAssignmntId(classId, assignMentIdObj[testIds[0]])
              : k === 1
              ? egp.verifyClassAndAssignmntId(classId, assignMentIdObj[testIds[0]])
              : k === 2
              ? sbr.verifyClassAndAssignmntId(classId, assignMentIdObj[testIds[0]])
              : assert.fail();
            cy.go("back");
            cy.wait("@go-to-playList");
          }
      ...
      it("> verify assignments rows in favourite playlist page", () => {
        ...
        const rowData_1 = { status: teacherSide.IN_PROGRESS, submitted: "0 of 1", graded: "0", className: classes[0] };
        playlistlibraryPage.reviewTab.verifyAssignmentRowByTestByMod(1, 1, { ...commonData, ...rowData_1 });

        const rowData_2 = { status: teacherSide.IN_GRADING, submitted: "1 of 1", graded: "1", className: classes[1] };
        playlistlibraryPage.reviewTab.verifyAssignmentRowByTestByMod(1, 1, { ...commonData, ...rowData_2 }, 1);
      });
    });
  });
  ...
  context("> reassign tests with playlists", () => {
    context("> reassign test from playList", () => {
      ...
      it("> teacher side verification-'assignments page'", () => {
        ...
        authorAssignmentPage.getStatus().should("have.length", testCount);
        authorAssignmentPage.getAssignmentRowsTestById(testIds[0]).should("have.length", testCount);
        playlistlibraryPage.sidebar.clickOnRecentUsedPlayList();
      });
      it("> student side verification-'assignments button'", () => {
        cy.login("student", class1Student.email, class1Student.pass);
        assignmentsPage.getAssignmentButton().should("have.length", testCount);
        assignmentsPage.getAssignmentByTestId(testIds[0]).should("have.length", testCount);
      });
    });
    context("> reassign test-'already assigned test is part of a playlist'", () => {
