...
describe(`${FileHelper.getSpecName(Cypress.spec.name)}>> play list basics`, () => {
  ...
  context("> using existing playlist template", () => {
    ...
    it("> add modules", () => {
      ...
      playlistlibraryPage.reviewTab.clickAddNewModule();
      playlistlibraryPage.reviewTab.setModuleDetails("module3", "mod3", "module-3", "module-description-3");
    ...
    it("> publish and use playlist, verify new module details", () => {
      ...
      playlistlibraryPage.reviewTab.verifyModuleDetailsByModule(2, "module3", "mod3", "module-description-3");
    });
    ...
    it("> 'clone' in playlist library", () => {
      ...
      playlistlibraryPage.clickOnCloneOnCardByPlaylistId(playlistId_2).then(id => {
        expect(id, `playlist clone ${id === playlistId_2 ? "failed" : "success"}`).not.eq(playlistId_2);
        cy.url()
          .should("not.contain", playlistId_2)
          .should("contain", id);
      });
    ...
    it("> verify review tab", () => {
      ...
      testIds.slice(2).forEach((test, i) => {
        ...
        playlistlibraryPage.reviewTab.getTestByTestByModule(i + 1, 1).then(($ele, i) => {
