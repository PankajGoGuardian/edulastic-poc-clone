...
describe(`${FileHelper.getSpecName(Cypress.spec.name)}> Playlist customization`, () => {
  ...
  context("Search from search bar", () => {
    before("Go to Resource Container", () => {
      playlistlibraryPage.playlistCustom.clickOnManageContent();
      playlistlibraryPage.playlistCustom.searchContainer.setFilters({ authoredByme: true });
    });
    it("search By Name", () => {
      playlistlibraryPage.playlistCustom.searchContainer.typeInSearchBar(`${testName}`);
      playlistlibraryPage.playlistCustom.searchContainer
        .getTestInSearchResultsById(testId.testId1)
        .should("contain", `${testName}`);
    });
    it("Search by Id", () => {
      playlistlibraryPage.playlistCustom.searchContainer.typeInSearchBar(`${testId.testId2}`);
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId2}`);
    });
    it("Search By standard", () => {
      playlistlibraryPage.playlistCustom.searchContainer.typeInSearchBar(`${testStandard.standard1}`);
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId1}`);
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId2}`);

      playlistlibraryPage.playlistCustom.searchContainer.typeInSearchBar(`${testStandard.standard2}`, false);
      playlistlibraryPage.playlistCustom.searchContainer.getSearchContainer().should("contain", "test search 2 ");
    });
    it("Search by Tag", () => {
      playlistlibraryPage.playlistCustom.searchContainer.typeInSearchBar(`${testTag.tag1}`);
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId1}`);

      playlistlibraryPage.playlistCustom.searchContainer.typeInSearchBar(`${testTag.tag2}`);
      playlistlibraryPage.playlistCustom.searchContainer.getSearchContainer().should("contain", "No Data");
    });
  });
  ...
  context("search from Authored by me folder", () => {
    it("Verifiy grade,subject and status filter", () => {
      ...
      playlistlibraryPage.playlistCustom.searchContainer.setFilters({ grade: grades.GRADE_4 });
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId1}`);
      playlistlibraryPage.playlistCustom.searchContainer.VerififySearchResultNotVisible(`${testId.testId8}`);

      playlistlibraryPage.playlistCustom.searchContainer.setFilters({ status: "Draft" });
      playlistlibraryPage.playlistCustom.searchContainer.VerififySearchResultNotVisible(`${testId.testId8}`);
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId4}`);

      playlistlibraryPage.playlistCustom.searchContainer.setFilters({ status: "Published", subject: subject.MATH });
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId2}`);
      playlistlibraryPage.playlistCustom.searchContainer.VerififySearchResultNotVisible(`${testId.testId8}`);
    });
    it("Verify collection dropdwon", () => {
      playlistlibraryPage.playlistCustom.searchContainer.setFilters({ collection: collection.col1 });
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId1}`);
      playlistlibraryPage.playlistCustom.searchContainer.VerififySearchResultNotVisible(`${testId.testId2}`);

      playlistlibraryPage.playlistCustom.searchContainer.setFilters({ collection: collection.col2 });
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId2}`);
      playlistlibraryPage.playlistCustom.searchContainer.VerififySearchResultNotVisible(`${testId.testId3}`);

      playlistlibraryPage.playlistCustom.searchContainer.setFilters({ collection: collection.col3 });
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId3}`);
      playlistlibraryPage.playlistCustom.searchContainer.VerififySearchResultNotVisible(`${testId.testId2}`);

      playlistlibraryPage.playlistCustom.searchContainer.setFilters({ collection: collection.col4 });
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId3}`);
      playlistlibraryPage.playlistCustom.searchContainer.VerififySearchResultNotVisible(`${testId.testId6}`);
    });
  });
  ...
  context("search from Entire Library folder", () => {
    ...
    it("Verify grade, subject", () => {
      playlistlibraryPage.playlistCustom.searchContainer.setFilters({ entireLibrary: true, status: "All" });
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId3}`);
      playlistlibraryPage.playlistCustom.searchContainer.VerififySearchResultNotVisible(`${testId.testId4}`);

      playlistlibraryPage.playlistCustom.searchContainer.setFilters({ grade: grades.GRADE_8 });
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId3}`);
      playlistlibraryPage.playlistCustom.searchContainer.VerififySearchResultNotVisible(`${testId.testId8}`);
      playlistlibraryPage.playlistCustom.searchContainer.setFilters({ subject: subject.MATH });

      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId5}`);
      playlistlibraryPage.playlistCustom.searchContainer.VerififySearchResultNotVisible(`${testId.testId8}`);
    });
  });
  ...
  context("search from Shared with me folder", () => {
    it("Shared with Me", () => {
      ...
      playlistlibraryPage.playlistCustom.searchContainer.setFilters({ SharedWithMe: true, grade: grades.GRADE_8 });
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId6}`);

      playlistlibraryPage.playlistCustom.searchContainer.setFilters({ SharedWithMe: true, subject: subject.ELA });
      playlistlibraryPage.playlistCustom.searchContainer.verifySearchResultVisible(`${testId.testId7}`);
    });
  });
  ...
  context("add / search resources", () => {
    ...
    before("> delete sub resources", () => {
      ...
      playlistlibraryPage.playlistCustom.getAllSubResourceRowsByMod(1).should("not.have.descendants");

      playlistlibraryPage.playlistCustom.clickExpandByModule(2);
      playlistlibraryPage.playlistCustom.deleteAllSubResourceRows(2);
      playlistlibraryPage.playlistCustom.getAllSubResourceRowsByMod(2).should("not.have.descendants");
    });
    ...
    resources.forEach((resource, i) => {
      it(`> add resource '${resource}'`, () => {
        ...
        playlistlibraryPage.playlistCustom.searchContainer.clickOptionInDropDownByText(resource);
        playlistlibraryPage.playlistCustom.searchContainer.setInformationInAddResourcePopUp(details);
        playlistlibraryPage.playlistCustom.searchContainer.clickAddResourceInPopUp().then(id => {
    ...
    it("> search new resource", () => {
      resources.forEach((resource, i) => {
        playlistlibraryPage.playlistCustom.searchContainer.typeInSearchBar(`${resource}-${randomString.slice(0, 3)}`);
        playlistlibraryPage.playlistCustom.searchContainer
    ...
    it("> search existing resource", () => {
      resources.forEach((resource, i) => {
        playlistlibraryPage.playlistCustom.searchContainer.typeInSearchBar(`${resource}-existing`);
        playlistlibraryPage.playlistCustom.searchContainer
          .getTestInSearchResultsById(newResourceId[i])
          .should("be.visible");
      });
    ...
    it("> add resources to test as sub-resource", () => {
      [oldresourceId[0], newResourceId[1]].forEach((resource, i) => {
        playlistlibraryPage.playlistCustom.searchContainer.typeInSearchBar(resource);
        playlistlibraryPage.playlistCustom.dragResourceFromSearchToModule(i + 1, 1, resource);
      });
    ...
    it("> verify resources after navigations", () => {
      ...
      [oldresourceId[0], newResourceId[1]].forEach((resource, i) => {
        ...
        playlistlibraryPage.playlistCustom.getSubResourceByTestByMod(i + 1, 1, resource);
      });
    });
  });
  ...
  context("> customization page-'own playlist'", () => {
    ...
    before("> create a playlist", () => {
      ...
      playlistlibraryPage.createPlayListWithTests({ metadata: playListData, moduledata }).then(id => {
        ...
        playlistlibraryPage.clickOnCloneOnCardByPlaylistId(playlistId_1).then(id => {
          playlistId_2 = id;
    ...
    it("> remove from favourites", () => {
      ...
      playlistlibraryPage.reviewTab.getPlaylistCardById(playlistId_1).should("not.exist");
    ...
    it("> manage test dropdown 'Preview Test'", () => {
      ...
      playlistlibraryPage.reviewTab.clickOnViewTestByTestByModule(1, 1, true).then(test => {
    ...
    it("> manage test dropdown 'view test details'", () => {
      ...
      playlistlibraryPage.reviewTab.clickOnViewDetailsInDropDownByTestByMod(1, 1, true);
    ...
    it("> manage test dropdown 'unassign test'", () => {
      ...
      playlistlibraryPage.reviewTab.clickOnUnAssignInDropDownByTestByModule(1, 1);
      playlistlibraryPage.sidebar.clickOnAssignment();
