...
describe(`${FileHelper.getSpecName(Cypress.spec.name)}>> playlist overide settings`, () => {
  ...
  before("create test", () => {
    ...
    [1, 2].forEach((test, i) => {
      ...
      if (i === 0) testlibraryPage.testSettings.clickOnCalculatorByType(CALCULATOR.BASIC);
      else testlibraryPage.testSettings.setCheckAnswer(3);

      testlibraryPage.testSettings.setRealeasePolicy(releaseGradeTypes.SCORE_ONLY);

      testlibraryPage.testSettings.clickOnPassword();
      testlibraryPage.testSettings.clickOnStaticPassword();

      if (i === 0) testlibraryPage.testSettings.enterStaticPassword(staticPass[0]);
  ...
  context("> overide setting and verify", () => {
    ...
    it("> over ride settings", () => {
      ...
      playlistlibraryPage.playListAssign.clickOnCalculatorByType(CALCULATOR.NONE);
