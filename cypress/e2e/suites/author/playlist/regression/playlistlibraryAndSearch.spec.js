...
import { grades, subject as subjects } from "../../../../framework/constants/assignmentStatus";
import { getNumberInRange } from "../../../../framework/constants/constantFunctions";

describe(`${FileHelper.getSpecName(Cypress.spec.name)}>> playlist library / search`, () => {
  ...
  context("> authored by me", () => {
    context("> list view", () => {
      before("> switch to list view", () => {
        ...
        playlistlibraryPage.searchFilter.typeInSearchBox(metadata[2].newPlaylistId);
      });

      it("> title, description", () => {
        const { name, description, newPlaylistId } = metadata[2];
        testlibraryPage.verifyDescriptionOnTestCardById(newPlaylistId, description);
    ...
    context("> tile view", () => {
      before("> switch to list view", () => {
        ...
        playlistlibraryPage.searchFilter.clearAll();
        playlistlibraryPage.searchFilter.typeInSearchBox(metadata[2].existingPlaylistId);
      });

      it("> title, description and tags", () => {
        const { name, description, tags, existingPlaylistId } = metadata[2];
        playlistlibraryPage.verifyDescriptionOnPlaylistById(existingPlaylistId, description);
      ...
      it("> id, author name", () => {
        ...
        testlibraryPage.verifyAuthorNameOnTestCardById(existingPlaylistId, "Teacher");
      });
    });

    context("> search using search bar", () => {
      it("> search by 'name'", () => {
        metadata.slice(0, 2).forEach(({ name, existingPlaylistId, newPlaylistId }) => {
          playlistlibraryPage.searchFilter.clearAll();
          ...
          testlibraryPage.getAllTestCardsInCurrentPage().should("have.length", 2);
        });
      });

      it("> search by 'grade'", () => {
        metadata.slice(0, 2).forEach(({ grade, existingPlaylistId, newPlaylistId }) => {
          playlistlibraryPage.searchFilter.clearAll();
          playlistlibraryPage.searchFilter.getAuthoredByMe();
      ...
      it("> search by 'id'", () => {
        metadata.slice(0, 1).forEach(({ existingPlaylistId, newPlaylistId }) => {
          [existingPlaylistId, newPlaylistId].forEach((_id, i) => {
            playlistlibraryPage.searchFilter.clearAll();
            playlistlibraryPage.searchFilter.getAuthoredByMe();
            ...
            playlistlibraryPage.getPlayListCardById(_id);
            testlibraryPage.getAllTestCardsInCurrentPage().should("have.length", 1);
          });
        });
      });

      it("> search by 'tag'", () => {
        metadata.slice(0, 2).forEach(({ tags, existingPlaylistId, newPlaylistId }) => {
          playlistlibraryPage.searchFilter.clearAll();
          playlistlibraryPage.searchFilter.getAuthoredByMe();
      ...
      it("> search by 'name + tag'", () => {
        metadata.slice(0, 2).forEach(({ tags, name, existingPlaylistId, newPlaylistId }) => {
          playlistlibraryPage.searchFilter.clearAll();
          playlistlibraryPage.searchFilter.getAuthoredByMe();
          playlistlibraryPage.searchFilter.typeInSearchBox(tags[0]);
          playlistlibraryPage.searchFilter.typeInSearchBox(name);
    ...
    context("> search using filters", () => {
      ...
      it("> using 'subjects'", () => {
        metadata.slice(1).forEach(({ subject, existingPlaylistId, newPlaylistId }) => {
      ...
      it("> using 'tags'", () => {
        metadata.slice(1).forEach(({ tags, existingPlaylistId, newPlaylistId }) => {
          playlistlibraryPage.searchFilter.clearAll();
          playlistlibraryPage.searchFilter.getAuthoredByMe();
      ...
      it("> using collection- 'private library'", () => {
        playlistlibraryPage.searchFilter.clearAll();
        playlistlibraryPage.searchFilter.setFilters({ collection: COLLECTION.private }, false);

        metadata.forEach(({ existingPlaylistId, newPlaylistId }) => {
          [existingPlaylistId, newPlaylistId].forEach((_id, i) => {
            playlistlibraryPage.getPlayListCardById(_id);
          });
          testlibraryPage.getAllTestCardsInCurrentPage().should("have.length", 6);
        });
      });

      it("> using 'grade+subject'", () => {
        metadata.slice(0, 2).forEach(({ grade, subject, existingPlaylistId, newPlaylistId }) => {
      ...
      it("> using 'tags+subject'", () => {
        metadata.slice(0, 2).forEach(({ tags, subject, existingPlaylistId, newPlaylistId }) => {
          const standards = { subject };
          playlistlibraryPage.searchFilter.clearAll();
          playlistlibraryPage.searchFilter.getAuthoredByMe();
          playlistlibraryPage.searchFilter.setFilters({ standards, tags }, false);
