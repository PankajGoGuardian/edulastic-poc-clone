...
describe(`${FileHelper.getSpecName(
  Cypress.spec.name
)} >> Author "Match list" type question`, () => {
  const queData = {
    group: 'Classify, Match & Order',
    queType: questionType.MATCH_LIST,
    ...
    context(' > TC_76 => Set Correct Answer(s)', () => {
      ...
      it(' > Drag and drop the answer choices inside the box', () => {
        queData.choices.forEach((ch, index) => {
          ...
          cy.get(`.drag-drop-item-match-list`).last().trigger('dragstart', {})
  ...
  context(' > Scoring block test', () => {
    ...
    it(' > Test score with alternate and min score if attempted', () => {
      ...
      question.getPontsInput().clear().type('9{del}')

      question.addAlternate()

      question.getAddedAlternate().parent().click()
    ...
    it(' > Test score with partial match and panalty', () => {
      question
        ...
        .should('not.exist')
      question.getPontsInput().clear().type('{selectall}9')
      ...
      question.getPanalty().clear().type(`{selectall}3`)
