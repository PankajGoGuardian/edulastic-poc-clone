...
describe(`${FileHelper.getSpecName(
  Cypress.spec.name
)} >> Author "Cloze with Drag & Drop" type question`, () => {
  const queData = {
  ...
  context(' > User creates question', () => {
    ...
    context(' > TC_3 => Use Group possible responses block', () => {
      ...
      it(' > Check the group possible responses checkbox', () => {
        ...
        question.getAddedGroupTitle().next().click().should('not.exist')
    ...
    context(' > TC_4 => Set Correct Answer(s)', () => {
      it(' > Update Points', () => {
        question.getPontsInput().clear().type(`{selectall}1`)
        ...
        question
          ...
          .then(() => {
            question
              ...
              .contains('div', queData.choices[0])
          })
      ...
      it(' > Check/Uncheck Show Drag Handle', () => {
        ...
        question.getDraghandleCheck().find('input').should('be.checked')
        question.getResponseItemByIndex(0).find('i').should('be.visible')

        question.getDraghandleCheck().click()
        question.getDraghandleCheck().find('input').should('not.checked')
        question.getResponseItemByIndex(0).find('i').should('not.exist')
      })

      it(' > Check/uncheck Shuffle Options', () => {
        question.getShuffleOptionCheck().click()
        question.getShuffleOptionCheck().find('input').should('be.checked')
        question.header.preview()
        question.verifyShuffledChoices(queData.choices)
        question.header.edit()

        question.getShuffleOptionCheck().click()
        question.getShuffleOptionCheck().find('input').should('not.checked')
        question.header.preview()
      ...
      it(' > Click on Clear', () => {
        preview
          ...
          .then(() => {
            question
              .getResponseContainerByIndex(0)
              .should('not.contain', queData.choices[0])
            queData.choices.forEach((ch, index) => {
  ...
  context(' > Edit the question created', () => {
    ...
    context(' > TC_10 => Set Correct Answer(s)', () => {
      it(' > Update Points', () => {
        question.getPontsInput().type('{selectall}5').should('have.value', '5')
      })
      ...
      it(' > Check/Uncheck duplicate response checkbox', () => {
        ...
        question
          ...
          .then(() => {
            question
              .getResponseItemByIndex(0)
              .contains(queData.editedchoices[0])
          })
        ...
        question
          ...
          .then(() => {
            question
              .getResponseItemByIndex(0)
              .contains(queData.editedchoices[1])
          })
      })
  ...
  context(' > Scoring block tests', () => {
    ...
    it(' > Test scoring with alternate answer', () => {
      ...
      question
        .getPreviewAnswerBoxContainerByIndex(0)
        .should('have.css', 'background-color', queColor.LIGHT_GREEN)
      question
        .getPreviewAnswerBoxContainerByIndex(1)
        .should('have.css', 'background-color', queColor.LIGHT_GREEN)
