...
describe(`${FileHelper.getSpecName(
  Cypress.spec.name
)} >> Author "Cloze with Text" type question`, () => {
  const queData = {
  ...
  context(' > Create basic question and validate.', () => {
    ...
    context(
      ' > [clz_txt_s1] : user create basic question with default option and save',
      () => {
    ...
    context(
      ' > [clz_txt_s2] : preview and validate with right/wrong ans',
      () => {
        ...
        it('> Click on Clear', () => {
          preview.clickOnClear().then(() => {
            ...
            question
              .getResponseBoxByIndex(0)
              .should('have.css', 'background-color', queColor.WHITE_1)
          })
    ...
    context(
      ' > [clz_txt_s3] : Check Additional Options with multiple text inputs',
      () => {
