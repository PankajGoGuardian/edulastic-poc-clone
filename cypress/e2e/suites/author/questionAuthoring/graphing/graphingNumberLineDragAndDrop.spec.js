...
describe(`${FileHelper.getSpecName(
  Cypress.spec.name
)} >> Author "Number line with drag & drop" type question`, () => {
  const queData = {
    group: 'Graphing',
  ...
  context(' > User creates question.', () => {
    ...
    it(' > Edit line', () => {
      question.getXMinParameter().clear().type(queData.line.minValue)

      question.getXMaxParameter().clear().type(queData.line.maxValue)

      question.getVisibleTickLabelsOnBoard().should('have.length', 10)
    })

    it(' > Edit title', () => {
      question.getTitleParameter().clear().type(queData.title)
    ...
    it('Edit layout', () => {
      ...
      question.getLayoutLineMargin().clear().type(queData.layout.lineMargin)

      question.getLayoutLinePosition().clear().type(queData.layout.linePosition)
    ...
    it(' > Edit ticks', () => {
      question.getTicksDistance().clear().type('2').should('have.value', '2')
      question.getVisibleTickLabelsOnBoard().should('have.length', 6)

      question.getTicksDistance().clear().type('1').should('have.value', '1')
      question.getVisibleTickLabelsOnBoard().should('have.length', 10)
    ...
    it(' > Edit labels', () => {
      question.getLabelsFrequency().clear().type(queData.labels.frequency)
    ...
    it(' > Edit correct answers', () => {
      ...
      question
        .getMountedMark(queData.possibleResponses.respA)
        .should('be.exist')
      question
        .getMountedMark(queData.possibleResponses.respB)
        .should('be.exist')
      question
        .getMountedMark(queData.possibleResponses.respC)
        .should('be.exist')
