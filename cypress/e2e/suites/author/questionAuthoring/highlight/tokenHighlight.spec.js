...
describe(`${FileHelper.getSpecName(
  Cypress.spec.name
)} >> Author "Token highlight" type question`, () => {
  const queData = {
    group: 'Highlight',
  ...
  context(' > User creates question', () => {
    ...
    it(' > [Tc_211] : Edit template and token', () => {
      ...
      const tokens = [
        { sel: 'Paragraph', count: 2 },
        { sel: 'Sentence', count: 5 },
        { sel: 'Word', count: 20 },
      ]
    ...
    context(
      ' > [Tc_214] :  preview and validate checkAns,ShowAns,Clear',
      () => {
        ...
        it(' > [Tc_2] : Sentence Token, validate checkAns', () => {
          ...
          question.getAllTokenAnswer().then(($ele) => {
            queData.correct.sentence.forEach((text) => {
              cy.wrap($ele).contains(text).click()
            })
          })
