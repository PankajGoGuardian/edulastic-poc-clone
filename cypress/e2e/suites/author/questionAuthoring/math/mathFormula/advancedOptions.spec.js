...
describe(`${FileHelper.getSpecName(
  Cypress.spec.name
)} >> Extras on "Math formula" type question`, () => {
  const queData = {
    group: 'Math',
 ...
 */
      describe(' > Main options', () => {
        ...
        it(' > Enter rubric reference text', () => {
          ...
          extras.getRubricReference().clear().type(text).should('contain', text)
        })
        ...
        it(' > Enter sample answer text', () => {
          ...
          extras.getSampleAnswer().clear().type(text).should('contain', text)
        })
      })
