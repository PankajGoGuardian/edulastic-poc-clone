...
describe('Author - Multiple choice - standard type question', () => {
  const queData = {
 ...
 */
    ...
    it('[Tc_252]:test => Set Correct Answer(s)', () => {
      ...
      question.getAllAnsChoicesLabel()
        .find('input:checked')
    ...
    it('[Tc_253]:test => Advanced Options', () => {
      ...
      question.getEnableAutoScoring()
        .click()
        .then(($el) => {
          cy.wrap($el)
            .should('have.class', 'ant-checkbox-checked');

          question.getCheckAnswerCheckbox()
            .click()
            .should('have.class', 'ant-checkbox-checked')
            .click()
    ...
    it('[Tc_255]:test => Save question', () => {
      editItem.header.save();
      cy.contains(queData.formattext)
        .should('be.visible');
      cy.url()
        .should('contain', 'item-detail');
    });
  ...
  context('[sanity]:test => Create question using different options and validate', () => {
    ...
    it('[mcq_std_test1]:test => Validate basic question with default setting', () => {
      ...
      preview.getShowAnswer()
        .click().then(() => {
          cy.get('label.wrong')
            .should('have.length', queData.choices.length - 1);
      ...
      preview.getClear()
        .click().then(() => {
          cy.get('label.right,label.wrong')
            .should('have.length', 0);
        });

      // give correct ans and validate
      cy.contains(queData.correct[0])
        .click();
    ...
    it('[mcq_std_test2]:test => Enable multiple responses (exact & partial match) and validate', () => {
      ...
      preview.getShowAnswer()
        .click().then(() => {
          cy.get('label.wrong')
            .should('have.length', queData.choices.length - 2);
      ...
      cy.contains(queData.alterate[0])
        .click();
