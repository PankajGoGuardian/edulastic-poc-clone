    ...
    context(`method-${method}`, () => {
      ...
      testCase[method].forEach((testcase, i) => {
        ...
        it(` > testcase-${i + 1}-${testcaseId}:${name || 'no_name'}`, () => {
                    ...
                    if (_.isObject(optionValue)) {
                      ...
                      if (ruleToSet === 'isDecimal')
                        question
                          ...
                          .clear({ force: true })
                          .type(`{selectall}${optionValue.isDecimal}`, {
                            force: true,
                          })

                      if (ruleToSet === 'isStandardForm')
                        question.setAnswerArgumentDropdownValue(
                          optionValue.isStandardForm
                        )
          ...
          resetInput = testCase[method][i + 1]
            ? testCase[method][i + 1].input !== testCase[method][i].input
            : true
          ...
          question
            ...
            .typeWithDelay(attempt, { force: true })
          ...
          preview.checkOnCheckAnswer().then(() => {
            cy.wait('@evaluate').then((xhr) => {
              ...
              expect(
                xhr.status,
                `verify evaluation status - ${xhr.status} - ${
                  xhr.status === 200 || JSON.stringify(xhr.responseBody)
                }`
              ).to.eq(200)

              expect(
                xhr.response.body.correct,
                `${JSON.stringify(xhr.responseBody.testcase)}`
              ).eq(evaluation)
              ...
              preview.getAntMsg().then(($ele) => {
                ...
                expect(
                  message,
                  `verify score message - ${message} - ${
                    message === `score: ${evaluation ? 1 : 0}` ||
