...
describe(`> ${FileHelper.getSpecName(
  Cypress.spec.name
)}> teacher marks redirected attempt as submit`, () => {
  ...
  const { _ } = Cypress
  const teacher = 'teachersubmits@redirect.com'
  const students = [
    {
      email: 'tsubmit16198',
      name: 'Submit1, Tea',
  ...
  const testId = '5f8aaaa60494ee00082b688a'
  const { itemKeys } = testData.TEST_PREVIEW
  ...
  before('> create a test', () => {
    ...
    /*  testlibraryPage.createTest('TEST_PREVIEW', false).then((id) => {
  ...
  context(`> redirect with '${questionDeliveryType.All}'`, () => {
    ...
    3. attempt2:
          ...
          stu3: not started
    4. teacher marks all students as submitted      
    ...
    */
    before('> create and assign test', () => {
      ...
      testlibraryPage.assignPage.selectClass('rediect teacher submit')
