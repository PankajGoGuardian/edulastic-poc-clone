...
describe(`>${FileHelper.getSpecName(
  Cypress.spec.name
)}> regrade settings- check answer tries`, () => {
  ...
  const students = {
    class1: {
      1: {
        email: 's1.c1.regrade.checkanstries@snapwiz.com',
        status: studentSide.IN_PROGRESS,
        name: 'Stu1Class1',
      },
      2: {
        email: 's2.c1.regrade.checkanstries@snapwiz.com',
    ...
    class2: {
      1: {
        email: 's1.c2.regrade.checkanstries@snapwiz.com',
        status: studentSide.IN_PROGRESS,
        name: 'Stu1Class2',
      },
      2: {
        email: 's2.c2.regrade.checkanstries@snapwiz.com',
        status: studentSide.NOT_STARTED,
        name: 'Stu2Class2',
      },
    },
  ...
  context(`> select '${regradeOptions.settings.excludeOveridden}'`, () => {
    ...
    */
    ...
    context('> edit settings and regrade', () => {
      ...
      context(
        `> verify regrdaded no of check ans attempts at student side`,
        () => {
          ;[...attemptsdata1, ...attemptsdata2].forEach(
            ({ email, overidden, status }, index) => {
              const [titleAdjust, maxChecks] = overidden
                ? ['', checkAns1]
                : ['not ', checkAns2]
  ...
  context(`> select '${regradeOptions.settings.chooseAll}'`, () => {
    ...
    */
    ...
    context('> edit settings and regrade', () => {
      ...
      context(
        `> verify regraded no of check ans attempts at student side`,
        () => {
          ;[...attemptsdata1, ...attemptsdata2].forEach(
            ({ email, overidden, status }, index) => {
