...
describe(`>${FileHelper.getSpecName(
  Cypress.spec.name
)}> regrade settings- 'performance band'`, () => {
  ...
  const students = {
    class1: {
      1: {
        email: 's1.c1.regrade.perfband@snapwiz.com',
        status: studentSide.SUBMITTED,
        name: 'Stu1Class1',
      },
      2: {
        email: 's2.c1.regrade.perfband@snapwiz.com',
        status: studentSide.IN_PROGRESS,
        name: 'Stu2Class1',
      },
      3: {
        email: 's3.c1.regrade.perfband@snapwiz.com',
    ...
    class2: {
      1: {
        email: 's1.c2.regrade.perfband@snapwiz.com',
        status: studentSide.SUBMITTED,
        name: 'Stu1Class2',
      },
      2: {
        email: 's2.c2.regrade.perfband@snapwiz.com',
        status: studentSide.IN_PROGRESS,
        name: 'Stu2Class2',
      },
      3: {
        email: 's3.c2.regrade.perfband@snapwiz.com',
        status: studentSide.NOT_STARTED,
        name: 'Stu3Class2',
      },
    },
    ...
    */
    ...
    context('> edit settings and regrade', () => {
      ...
      ;[attemptsdata1, attemptsdata2].forEach((studentdata) => {
        context(
          `> verify lcb/reports '${
            studentdata[0].overidden === 0 ? 'not ' : ''
          }overidden' assignment`,
          () => {
            before('> click on lcb', () => {
              const assignmentid = studentdata[0].overidden
                ? assignmentid2
                : assignmentid1
              testlibaryPage.sidebar.clickOnAssignment()
              authorAssignmentPage.clickOnLCBbyTestId(
            ...
            context('> verify reports', () => {
              ...
              studentdata.forEach(({ status, name, attempt }) => {
                it(`> for student ${status}`, () => {
                  ...
                  pdfReportCard
                    .getPerformanceBand()
                    .should('have.text', bands[attempt])
                })
              })
            })
          }
