...
describe(`>${FileHelper.getSpecName(
  ...
  const students = {
    class1: {
      1: {
        email: 's1.c1.regrade.shuffle.que@snapwiz.com',
        status: studentSide.SUBMITTED,
        name: 'Stu1Class1',
      },
      2: {
        email: 's2.c1.regrade.shuffle.que@snapwiz.com',
        status: studentSide.IN_PROGRESS,
        name: 'Stu2Class1',
      },
      3: {
        email: 's3.c1.regrade.shuffle.que@snapwiz.com',
    ...
    class2: {
      1: {
        email: 's1.c2.regrade.shuffle.que@snapwiz.com',
        status: studentSide.SUBMITTED,
        name: 'Stu1Class2',
      },
      2: {
        email: 's2.c2.regrade.shuffle.que@snapwiz.com',
        status: studentSide.IN_PROGRESS,
        name: 'Stu2Class2',
      },
      3: {
        email: 's3.c2.regrade.shuffle.que@snapwiz.com',
        status: studentSide.NOT_STARTED,
        name: 'Stu3Class2',
      },
    },
  ...
  const groups = {
    1: { deliverType: deliverType.ALL, deliveryCount: 4, items: itemSeqInTest },
  }
              ...
              studentdata.forEach((stu, ind) => {
                ...
                itemSeqInTest.forEach((id, ind) => {
                  lcb.questionResponsePage
                    ...
                    .should('contain', `Q${ind + 1}${queData.queString}`)
                })
              })
            })
        ...
        context(
          ...
          () => {
            ...
            before('> click on lcb', () => {
              ...
              authorAssignmentPage.clickOnLCBbyTestId(
                versionedTest1,
                studentdata[0].overidden ? assignid2 : assignid1
              )
            })
      ...
      testlibaryPage.header.clickOnPublishButton().then((id) => {
        ...
        ;[...attemptsdata1, ...attemptsdata2]
          ...
          .forEach(({ email, status, attempt, overidden }) => {
            ...
            studentTestPage.assignmentPage
              .clickOnAssigmentByTestId(test2)
              .then((deliveredItemGroups) => {
                const deliveredSeq = groupItemsPage.getItemDeliverySeq(
                  deliveredItemGroups,
                  groups,
        ...
        context(
          ...
          () => {
            before('> login and click on lcb by assignment id', () => {
              ...
              authorAssignmentPage.clickOnLCBbyTestId(
                versionedTest2,
                classIndex === 0 ? assignid1 : assignid2
              )
            })
                ...
                .then((deliveredItemGroups) => {
                  ...
                  deliveredSeq.forEach((id, ind) => {
                    ...
                    studentTestPage
                      ...
                      .should(
                        'contain',
                        `Q${currentQuesIndex + 1} - This is MCQ_TF.2`
                      )
                    studentTestPage.clickOnNext(false, true)
                  })
                  studentTestPage.submitTest()
