  ...
  context('> in test review', () => {
    before(' > clone test along with items', () => {
      testLibraryPage.createTest(TEST, true, originalItemId).then((id) => {
        ...
        testLibraryPage.selectCloneOptionAndCloneTest(true).then((ids) => {
          clonedTestId = ids.testId
          clonedItems = ids.itemIds
          totalItemsInItemBank += clonedItems.length
        })
      })
    })
    ...
    it('> verify test summary in review tab', () => {
      ...
      testLibraryPage.review.verifySummary(clonedItems.length, totalPoints)
    ...
    it('> verify item bank', () => {
      ...
      ;[...originalItemId, ...clonedItems].forEach((id) => {
