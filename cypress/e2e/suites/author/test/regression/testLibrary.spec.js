...
describe(`${FileHelper.getSpecName(Cypress.spec.name)} >> test library`, () => {
  ...
  before('> get standards', () => {
    _.keys(testData).forEach((tests) => {
      ...
      lcb.getQuestionTypeMap(
        testData[tests].items,
        questionData,
        questionTypeMap
      )
      testData[
        tests
      ].standardTableData = studentsReportCard.getStandardTableData({
        stuAttempt,
        questionTypeMap,
      })
    })
  ...
  before('> filter mapping', () => {
    _.keys(testData).forEach((tests) => {
      ...
      testData[tests].items.forEach((item) => {
        ...
        testGrades = _.union(
          [questionData[queType][index].standards[0].grade],
          testGrades
        )
        standardSet = _.union(
          [questionData[queType][index].standards[0].standardSet],
          standardSet
        )
  ...
  context('> test card in tile view', () => {
    ...
    it('> verify standards and collection', () => {
      ...
      testLibraryPage.verifyCollectionOnTestCardbyId(
        id,
        COLLECTION.private.split(' ')[0].toUpperCase()
      )
    })
  ...
  context('> test card in pop up view', () => {
    ...
    it('> verify test name,description and collection', () => {
      ...
      testLibraryPage.verifyTestCollectionOnTestCardPopUp(
        COLLECTION.private.split(' ')[0].toUpperCase()
      )
    })
  ...
  context('> paginations', () => {
    ...
    it('> jump to next 5 pages and previous 5 pages', () => {
      testLibraryPage.searchFilters.clickButtonInPaginationByPageNo(
        'Next 5 Pages'
      )
  ...
  context('> filters', () => {
    context('> authored by me', () => {
      ...
      it('> grades', () => {
        const customFilter = {
          standards: { grade: [GRADES.GRADE_10, GRADES.KINDERGARTEN] },
        }
        const filteredTest = _.values(testData).filter(
          (a) =>
            _.intersection(customFilter.standards.grade, a.testGrades).length >
            0
        )
        ...
        testLibraryPage
          ...
          .should('have.length', filteredTest.length)
      })

      it('> subjects', () => {
        const customFilter = { standards: { subject: subject.MATH } }
        const filteredTest = _.values(testData).filter(
          (a) => a.subjects.indexOf(customFilter.standards.subject) !== -1
        )
      ...
      it('> standard', () => {
        ...
        const filteredTest = _.values(testData).filter(
          (a) =>
            _.intersection(
              _.keys(a.standardTableData),
              customFilter.standards.standard
            ).length > 0
        )
      ...
      it('> collection', () => {
        ...
        testLibraryPage.searchFilters.clearAll()
        testLibraryPage.searchFilters.setFilters(customFilter, false)
        _.values(testData).forEach((test) =>
          testLibraryPage.getTestCardById(test.id)
        )
      })
      ...
      it('> tags', () => {
        ...
        const filteredTest = _.values(testData).filter(
          (a) => _.intersection(a.tags, customFilter.tags).length > 0
        )
      ...
      it('> grades + subject', () => {
        const customFilter = {
          standards: { grade: [GRADES.GRADE_10], subject: subject.ELA },
        }
        const filteredTest = _.values(testData).filter(
          (a) =>
            _.intersection(customFilter.standards.grade, a.testGrades).length >
              0 && a.subjects.indexOf(customFilter.standards.subject) !== -1
        )
      ...
      it('> grades + tags', () => {
        ...
        const filteredTest = _.values(testData).filter(
          (a) =>
            _.intersection(customFilter.standards.grade, a.testGrades).length >
              0 && _.intersection(customFilter.tags, a.tags).length > 0
        )
      ...
      it('> subjects', () => {
        ...
        randomIndices.forEach((index) => {
          testLibraryPage
            ...
            .then((id) => {
              testLibraryPage.clickOnTestCardById(id)
              testLibraryPage.verifySubjectsOnTestCardPopUp([
                customFilter.standards.subject,
              ])
              testLibraryPage.clickCloseTestCardPopUp()
            })
        })
