...
describe(`${FileHelper.getSpecName(Cypress.spec.name)} >> test sharing`, () => {
  ...
  context(`>test sharing 'individually'`, () => {
    ...
    context(
      ">with 'Can View & Duplicate' permission with 'individual other district'",
      () => {
        // Testing the above context for View and duplicate permission
        context(">with 'draft' test", () => {
          before('>creating test without publishing', () => {
          ...
          before('>share the draft test', () => {
            testLibrary.header.clickOnShare()
            testLibrary.selectPeopletoshare(
    ...
    context(
      ">sharing with 'different permissions to two users at same time' in 'draft' state",
      () => {
    ...
    context(
      ">sharing with 'different permissions to two users at same time' in 'published' state",
      () => {
  ...
  context('>sharing school,district and public levels', () => {
    ...
    context(">sharing at 'school level'", () => {
      ...
      it(`>assert share and 'View & Duplicate' permission for '${dist1_school1[teacher2][name]}' from same school`, () => {
        ...
        testLibrary.verifyCollectionOnTestCardbyId(
          test_id,
          COLLECTION.school.split(' ')[0].toUpperCase()
        )
        testLibrary.clickOnTestCardById(test_id)
        testLibrary.verifyTestCollectionOnTestCardPopUp(
          COLLECTION.school.split(' ')[0].toUpperCase()
        )
        testLibrary.assertTestPublishedNoEdit(test_id)
      })
    ...
    context(">sharing at 'district level'", () => {
      ...
      it(`>assert share and 'View & Duplicate' permission for '${dist1_school2[teacher1][name]}' from other school`, () => {
        ...
        testLibrary.verifyCollectionOnTestCardbyId(
          test_id,
          COLLECTION.district.split(' ')[0].toUpperCase()
        )
        testLibrary.clickOnTestCardById(test_id)
        testLibrary.verifyTestCollectionOnTestCardPopUp(
          COLLECTION.district.split(' ')[0].toUpperCase()
