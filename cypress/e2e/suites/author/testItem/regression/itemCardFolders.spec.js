...
describe(`${FileHelper.getSpecName(Cypress.spec.name)}> item bank`, () => {
  ...
  const itemKeys = ["ESSAY_RICH.1", "ESSAY_RICH.2", "ESSAY_RICH.5", "MCQ_MULTI.6"];
  ...
  context(`> ques type, standards, author and item id on item cards in 'authored by me'`, () => {
    context(`> for existing item `, () => {
      before("> set filter", () => {
        ...
        itemlist.searchFilters.clearAll();
        itemlist.searchFilters.getAuthoredByMe();
        filters[1].queType = itemlist.mapQueTypeKeyToUITextInDropDown(filters[1].queType);
    ...
    context(`> for new item`, () => {
      before("> set filter", () => {
        ...
        itemlist.searchFilters.clearAll();
        itemlist.searchFilters.getAuthoredByMe();
        filters[3].queType = itemlist.mapQueTypeKeyToUITextInDropDown(filters[3].queType);
  ...
  context("> folder actions", () => {
    ...
    it("> create new folders", () => {
      ...
      [1, 2].forEach(folderIndex => {
        folderPage.getFolderByName(`Folder -${folderIndex}`).should("be.visible");
      });
    });
    ...
    it("> add items to folder", () => {
      ...
      CypressHelper.verifyAntMesssage(`2 item(s) successfully moved to "Folder -1"`);
    });
    ...
    it("> add used item to other folder along with new item", () => {
      ...
      CypressHelper.verifyAntMesssage(`${2} item(s) successfully moved to "Folder -2"`);
    });
    ...
    it("> remove from folder", () => {
      ...
      CypressHelper.verifyAntMesssage(`1 item(s) successfully removed from "Folder -2"`);
    });
    ...
    it("> try to add already added item", () => {
      ...
      CypressHelper.verifyAntMesssage(`1 item(s) already exist in Folder -1 folder`);
