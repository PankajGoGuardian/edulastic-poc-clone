  ...
  context(` > shuffle questions on`, () => {
    ...
    it(` > attempt with -  ${students[1].email}`, () => {
      ...
      Object.keys(itemKeys).forEach((que) => {
        ...
        studentTest
          .getQuestionText()
          .then((queText) => student1Sequence.push(queText.text()))
      ...
      studentTest.submitTest().then(() => {
        ...
        expect(
          !isSequenceMatch(originalSequence, student1Sequence),
    ...
    it(` > attempt with -  ${students[2].email}`, () => {
      ...
      Object.keys(itemKeys).forEach((que) => {
        ...
        studentTest
          .getQuestionText()
          .then((queText) => student2Sequence.push(queText.text()))
        studentTest.clickOnNext(false, true)
      })

      studentTest.submitTest().then(() => {
        expect(
          !isSequenceMatch(originalSequence, student2Sequence) ||
            !_.isEqual(student2Sequence, student1Sequence),
          'verify question seq is not same'
        ).to.eq(true)
      })
    })
    ...
    it(` > attempt with -  ${students[3].email}`, () => {
      ...
      Object.keys(itemKeys).forEach((que) => {
        ...
        studentTest
          .getQuestionText()
          .then((queText) => student3Sequence.push(queText.text()))
        studentTest.clickOnNext(false, true)
      })
      ...
      studentTest.submitTest().then(() => {
        expect(
          !isSequenceMatch(originalSequence, student3Sequence) ||
            !_.isEqual(student3Sequence, student1Sequence) ||
            !_.isEqual(student3Sequence, student2Sequence),
          'verify question seq is not same'
        ).to.eq(true)
      })
    })
      ...
      _.keys(students).forEach((stu) => {
        ...
        originalSequence.forEach((queText, q) => {
          lcb.questionResponsePage
            .getQuestionContainer(q)
            .should('contain.text', queText)
        })
      })
    })
