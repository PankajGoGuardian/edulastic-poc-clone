...
describe(`${FileHelper.getSpecName(
  ...
  context(' > Assignment attempt and stats', () => {
    ...
    describe(' > 1st attempt - test assessment player,attempt all questions and submit the test,validate', () => {
      it(' > verify button and drop down navigations', () => {
      ...
      it(' > attempt first 3 questions and verify resume', () => {
        // attempt 1st que
        // wrong ans
        cy.contains('Red').should('be.visible').click()
        test.checkAnsValidateAsWrong()
        // right ans
        cy.contains('Violet').should('be.visible').click()
        ...
        // attempt 2nd que
        // wrong ans
        cy.contains('TRUE').should('be.visible').click()
        test.checkAnsValidateAsWrong()
        // right ans
        cy.contains('true').should('be.visible').click()
        test.checkAnsValidateAsRight()
        ...
        test.checkAnsValidateAsWrong()
        // right ans
        cy.contains('False').should('be.visible').click()
        test.checkAnsValidateAsRight()
        ...
        // attempt 4th que
        // wrong
        cy.get('div.template_box > div.ant-select').eq(0).click()
      ...
      it(' > continue and submitted the assignemnt', () => {
        test.getQueDropDown().click()
        cy.contains('Question 4').should('be.visible').click()
        cy.contains('Q4').should('be.visible')
        // attempt 4th que
        // right
        cy.get('div.template_box > div.ant-select').eq(0).click()
    ...
    describe(' > 2nd attempt', () => {
      it(' > attempt questions,submit the test,validate stats', () => {
