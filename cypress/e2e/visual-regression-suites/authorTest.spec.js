    ...
    SCREEN_SIZES.forEach((size) => {
      context(`test-${testId}-${size}`, () => {
        ...
        it(`test-additem when resolution is - '${size}'`, () => {
          ...
          cy.wait('@testdetail')
          if (size[0] < screenSizes.MAX_MOBILE_WIDTH)
            testLibraryPage.header.clickOnfilters()
          testLibraryPage.searchFilters.clearAll()
          testLibraryPage.testAddItem.authoredByMe()
          if (size[0] < screenSizes.MAX_MOBILE_WIDTH)
            testLibraryPage.header.closeFilter()
          cy.wait(1000)
          cy.matchImageSnapshotWithSize()
        })
        ...
        it(`test-review expanded - when resolution is - '${size}'`, () => {
          ...
          cy.get('[data-cy="expandCollapseRow"]').should(
            'contain.text',
            'Collapse Rows'
          )
          cy.scrollTo(0, 0)
      ...
      context(`view as student-${testId}-${size}`, () => {
        ...
        testQuestions.forEach((itemId, i) => {
          ...
          it(`Q-${parseInt(i, 10) + 1}-${itemId}-resolution '${size}'`, () => {
            ...
            cy.contains(`${queNum}/ ${testQuestions.length}`).click({
              force: true,
            })
            cy.wait(2000) // allow que to render before taking screenshot
            cy.matchImageSnapshotWithSize()
            cy.get('[data-cy="assessment-player-default-wrapper"]').then(
              (ele) => {
                const wrapperHeight = ele.find('main').innerHeight()
                cy.isPageScrollPresent(60, wrapperHeight).then(
                  ({ hasScroll }) => {
                    if (hasScroll)
                      cy.scrollPageAndMatchImageSnapshots(
                        60,
                        wrapperHeight,
                        ele
                      )
