    ...
    Object.keys(json.testTypes).forEach((testKey) => {
      ...
      cy.request({
        ...
        cy.request({
          ...
          cy.writeFile(
            `${fixtureFolderPath}/studentsAttempt.json`,
            studentsAttemptJson
          )
        })
      })
...
Cypress.Commands.add(
  'deletePlayListRecommendation',
  (email, password = 'snapwiz') => {
...
Cypress.Commands.add('saveItemDetailToDelete', (itemId) => {
  if (itemId) {
    cy.readFile(`${deleteTestDataFile}`).then((json) => {
      ...
      if (!Cypress._.isArray(itemId)) itemId = [itemId]
      itemId.forEach((item) => {
        json.testItems.push({
          _id: item,
          authToken: token,
  ...
  cy.task('readFileContent', deleteTestDataFile).then((fileContent) => {
    ...
    if (fileContent !== null) {
      ...
      if (testData.testItems && testData.testItems.length > 0) {
        ...
        cy.wait(1).then(() => {
          if (failedToDeleteItems.length)
            testData.testItems = failedToDeleteItems
      ...
      cy.wait(1).then(() => {
        cleanupStateSuccess = !(
          failedToDeleteTests.length || failedToDeleteItems.length
        )
      })
    } else testData = {}
    ...
    cy.writeFile(deleteTestDataFile, testData).then(() => {
      expect(
        cleanupStateSuccess,
        `data clean-up stage failed - ${JSON.stringify(testData, null, 2)}`
      ).to.be.true
    })
  })
})
  ...
  if (userJson) {
    cy.readFile(`${deleteTestDataFile}`).then((json) => {
      ...
      if (!_.isArray(userJson._id)) json.users[userJson.role].push(userJson._id)
        ...
        json.users[userJson.role] = _.union(
          json.users[userJson.role],
          userJson._id
        )
      cy.writeFile(`${deleteTestDataFile}`, json)
    })
...
Cypress.Commands.add('deleteSchool', (school) => {
  cy.request({
    ...
    expect(status).to.eq(200)
    cy.log(
      'school removed with _id :',
      school.deleteBody.schoolId,
      ' for userId : ',
      school.deleteBody.userId
    )
  })
})
