...
Cypress.Commands.add('matchImageSnapshotWithSize', (filename) => {
  ...
  return cy.document().then((doc) => {
    ...
    const screenshotFile = `${width || doc.body.clientWidth}/${
      filename || testName
    }`
...
Cypress.Commands.add('isPageScrollPresent', (scrollOffset = 10, pageHeight) =>
  cy.document().then((doc) => {
    ...
    const scroll = {
      ...
      minScrolls:
        Cypress._.ceil(scrollHeight / (doc.body.clientHeight - scrollOffset)) -
        1,
...
Cypress.Commands.add(
  'setToken',
  (email = DEFAULT_USERS.teacher.username, password = 'snapwiz') => {
    const user = {
      password,
    }
...
Cypress.Commands.add(
  'login',
  (role = 'teacher', email, password = 'snapwiz') => {
    const postData = {}
    ...
    cy.wait(500).then(() => {
      cy.get('body').then(() => {
        ...
        cy.url().then((url) =>
          url.includes('/login') ? cy.reload() : cy.visit('/login')
        )
      })
    })
    cy.server()
...
Cypress.Commands.add(
  'uploadFile',
  (fileName, selector, subjectType = 'input') =>
    cy.get(selector).then((subject) => {
      cy.fixture(fileName, 'base64').then((fileContent) => {
        ...
        cy.wrap(subject)
          .upload(
            { fileContent, fileName, mimeType: getMimetype(fileName) },
            { subjectType }
          )
