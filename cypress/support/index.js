...
Cypress.on('test:after:run', (test, runnable) => {
  ...
  if (test.state === 'failed') {
    ...
    screenshotFileName = imgError
      ? `${screenshotFileName}.diff.png`
      : `${screenshotFileName} (failed).png`
    ...
    addContext(
      { test },
      { title: 'test case flow - ', value: FileHelper.getTestFlow() }
    )
    addContext(
      { test },
      { title: 'screenshot for failed test case', value: imgPath }
    )
    addContext(
      { test },
      { title: 'video playback', value: `../videos/${Cypress.spec.name}.mp4` }
    )
  }
})
