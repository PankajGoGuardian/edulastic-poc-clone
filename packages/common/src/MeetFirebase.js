...
export const useFirestoreRealtimeDocuments = (queryFn, args) => {
  ...
  useEffect(() => {
    const unsubscribeCallback = queryFn(db).onSnapshot(querySnapshot => {
      const queriedDocs = querySnapshot.map(_doc => ({ ..._doc.data(), __id: _doc.id }));
      setDocs(queriedDocs);
...
export const meetBroadcast = async ({ meetingID, classId, userId, questionItem = {} }) => {
  const docRef = await db.collection("MeetingUserQuestions").doc(`${meetingID}-${userId}`);

  docRef.get().then(doc => {
    if (doc.exists) {
      docRef.update({ questions: firebase.firestore.FieldValue.arrayUnion(questionItem) });
    } else {
      docRef.set({ meetingID, classId, userId, questions: [questionItem], evaluations: [] });
    }
  });
};
