...
var CompareShapes = /*#__PURE__*/function () {
  ...
  (0, _createClass2["default"])(CompareShapes, [{
    ...
    value: function compare(testId, trueId) {
      ...
      var trueShape = compareTestShapes ? this.testAnswer.find(function (item) {
    ...
    value: function comparePoints(testPoint, truePoint) {
      if (testPoint.x === truePoint.x && testPoint.y === truePoint.y && (this.ignoreLabels || labelsAreEqual(testPoint.label, truePoint.label))) {
        return {
          id: testPoint.id,
    ...
    value: function compareLines(testShape, trueShape) {
      ...
      if (!this.ignoreLabels) {
        if (labelsAreEqual(testShape.label, trueShape.label) && (this.comparePoints(testShapePointStart, trueShapePointStart).result && this.comparePoints(testShapePointEnd, trueShapePointEnd).result || this.comparePoints(testShapePointStart, trueShapePointEnd).result && this.comparePoints(testShapePointEnd, trueShapePointStart).result)) {
      ...
      if (testShapeFunc.getKoefA() === trueShapeFunc.getKoefA() && testShapeFunc.getKoefB() === trueShapeFunc.getKoefB() && testShapeFunc.getVerticalLineOffset() === trueShapeFunc.getVerticalLineOffset()) {
        return positiveResult;
      }
    ...
    value: function compareRays(testShape, trueShape) {
      ...
      if (!this.ignoreLabels) {
        if (labelsAreEqual(testShape.label, trueShape.label) && this.comparePoints(testShapePointStart, trueShapePointStart).result && this.comparePoints(testShapePointEnd, trueShapePointEnd).result) {
          return positiveResult;
        }
      ...
      testRayParams.koef = testRayParams.deltaX === 0 ? "NaN" : (testRayParams.deltaY / testRayParams.deltaX).toFixed(_constants.FractionDigits);
      var trueRayParams = {
        deltaX: trueShapePoints.x2 - trueShapePoints.x1,
        deltaY: trueShapePoints.y2 - trueShapePoints.y1
      };
      trueRayParams.koef = trueRayParams.deltaX === 0 ? "NaN" : (trueRayParams.deltaY / trueRayParams.deltaX).toFixed(_constants.FractionDigits);

      if (testShapePoints.x1 === trueShapePoints.x1 && testShapePoints.y1 === trueShapePoints.y1 && testRayParams.koef === trueRayParams.koef && Math.sign(testRayParams.deltaX) === Math.sign(trueRayParams.deltaX) && Math.sign(testRayParams.deltaY) === Math.sign(trueRayParams.deltaY)) {
        return positiveResult;
      }
    ...
    value: function compareCircles(testShape, trueShape) {
      ...
      if (testShapePoints.startX === trueShapePoints.startX && testShapePoints.startY === trueShapePoints.startY && testCircleRadius === trueCircleRadius) {
        return positiveResult;
      }
    ...
    value: function compareParabolas(testShape, trueShape) {
      ...
      if (testShapePoints.startX === trueShapePoints.startX && testShapePoints.startY === trueShapePoints.startY && testFunc.getKoefA() === trueFunc.getKoefA() && testFunc.getDirection() === trueFunc.getDirection()) {
        return positiveResult;
      }
    ...
    value: function compareParabolas2(testShape, trueShape) {
      ...
      if (!this.ignoreLabels) {
        if (labelsAreEqual(testShape.label, trueShape.label) && this.comparePoints(testShapeDirPoint1, trueShapeDirPoint1).result && this.comparePoints(testShapeDirPoint2, trueShapeDirPoint2).result && this.comparePoints(testShapeFocusPoint, trueShapeFocusPoint).result) {
          return positiveResult;
        }
      ...
      if (testShapeFocusPoint.x === trueShapeFocusPoint.x && testShapeFocusPoint.y === trueShapeFocusPoint.y && testFunc.getKoefA() === trueFunc.getKoefA() && testFunc.getKoefB() === trueFunc.getKoefB() && testFunc.getVerticalLineOffset() === trueFunc.getVerticalLineOffset()) {
        return positiveResult;
      }
    ...
    value: function compareTrigonometrics(testShape, trueShape) {
      ...
      var testNormalX = testShapePoints.endY < testShapePoints.startY ? testShapePoints.startX + testPeriod / 2 : testShapePoints.startX;
      var trueNormalX = trueShapePoints.endY < trueShapePoints.startY ? trueShapePoints.startX + truePeriod / 2 : trueShapePoints.startX;
    ...
    value: function comparePolygons(testShape, trueShape) {
      ...
      for (var i = 0; i < testPolygonPoints.length; i++) {
        if (testPolygonPoints[i].x === truePolygonPoints[0].x && testPolygonPoints[i].y === truePolygonPoints[0].y && (this.ignoreLabels || labelsAreEqual(testPolygonPoints[i].label, truePolygonPoints[0].label))) {
          startIndex = i;
          break;
        }
      }
      ...
      for (var _i = 0; _i < testPolygonPoints.length; _i++) {
        if (testPolygonPoints[_i].x === truePolygonPoints[_i].x && testPolygonPoints[_i].y === truePolygonPoints[_i].y && (this.ignoreLabels || labelsAreEqual(testPolygonPoints[_i].label, truePolygonPoints[_i].label))) {
          equalCount++;
        }
      }

      if (equalCount === truePolygonPoints.length && (this.ignoreLabels || labelsAreEqual(testShape.label, trueShape.label))) {
        return positiveResult;
      } // check reverse order
      ...
      for (var _i2 = 0; _i2 < testPolygonPoints.length; _i2++) {
        if (testPolygonPoints[_i2].x === truePolygonPoints[_i2].x && testPolygonPoints[_i2].y === truePolygonPoints[_i2].y && (this.ignoreLabels || labelsAreEqual(testPolygonPoints[_i2].label, truePolygonPoints[_i2].label))) {
          equalCount++;
        }
      }
    ...
    value: function compareEllipses(testShape, trueShape) {
      ...
      if (!this.ignoreLabels) {
        if (labelsAreEqual(testShape.label, trueShape.label) && this.comparePoints(testShapeLinePoint, trueShapeLinePoint).result && (this.comparePoints(testShapeFocusPoint1, trueShapeFocusPoint1).result && this.comparePoints(testShapeFocusPoint2, trueShapeFocusPoint2).result || this.comparePoints(testShapeFocusPoint1, trueShapeFocusPoint2).result && this.comparePoints(testShapeFocusPoint2, trueShapeFocusPoint1).result)) {
          return positiveResult;
        }
      ...
      var focusPointsAreMatched = testShapePoints.focusPoint1X === trueShapePoints.focusPoint1X && testShapePoints.focusPoint1Y === trueShapePoints.focusPoint1Y && testShapePoints.focusPoint2X === trueShapePoints.focusPoint2X && testShapePoints.focusPoint2Y === trueShapePoints.focusPoint2Y || testShapePoints.focusPoint1X === trueShapePoints.focusPoint2X && testShapePoints.focusPoint1Y === trueShapePoints.focusPoint2Y && testShapePoints.focusPoint2X === trueShapePoints.focusPoint1X && testShapePoints.focusPoint2Y === trueShapePoints.focusPoint1Y;
    ...
    value: function compareExponents(testShape, trueShape) {
      ...
      if (testShapePoints.startX === trueShapePoints.startX && testShapePoints.startY === trueShapePoints.startY && testFunc.getBC() === trueFunc.getBC()) {
        return {
          id: testShape.id,
          relatedId: trueShape.id,
          result: true
    ...
    value: function comparePolynoms(testShape, trueShape) {
      ...
      if (!this.ignoreLabels) {
        ...
        testShapePoints.forEach(function (testPoint) {
          equalCount += trueShapePoints.filter(function (truePoint) {
            return testPoint.x === truePoint.x && testPoint.y === truePoint.y && labelsAreEqual(testPoint.label, truePoint.label);
          }).length;
        });
      ...
      var allX = testShapePoints.map(function (point) {
        return point.x;
      }).concat(trueShapePoints.map(function (point) {
        return point.x;
      }));
