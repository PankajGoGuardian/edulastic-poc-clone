...
function () {
  ...
  (0, _createClass2.default)(CompareShapes, [{
    ...
    value: function compareRays(testRay, trueRay) {
      ...
      testRayParams.koef = testRayParams.deltaX === 0 ? 'NaN' : (testRayParams.deltaY / testRayParams.deltaX).toFixed(_fractionDigits.FractionDigits);
      var trueRayParams = {
        deltaX: trueRayPoints.x2 - trueRayPoints.x1,
        deltaY: trueRayPoints.y2 - trueRayPoints.y1
      };
      trueRayParams.koef = trueRayParams.deltaX === 0 ? 'NaN' : (trueRayParams.deltaY / trueRayParams.deltaX).toFixed(_fractionDigits.FractionDigits);
    ...
    value: function compareSines(testSine, trueSine) {
      ...
      var testNormalX = testSinePoints.endY < testSinePoints.startY ? testSinePoints.startX + testPeriod / 2 : testSinePoints.startX;
      var trueNormalX = trueSinePoints.endY < trueSinePoints.startY ? trueSinePoints.startX + truePeriod / 2 : trueSinePoints.startX;
