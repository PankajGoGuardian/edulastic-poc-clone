...
(async (cp, onlyToday) => {
  ...
  const throttledPromises = (asyncFunction, items = [], batchSize = 5, delay = 1000) =>
  ...
  const responses = await throttledPromises(a => Promise.all(a.map(_a => fetch(_a).then(__a => __a.json()))), promises);
  ...
  const title = ["Last Update", "Author", "Branch", "Status", "Url" /* , "Commits" */].join(";");
