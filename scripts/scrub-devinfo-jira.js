...
(async (cp, needCSVFormat) => {
  ...
  const promises = issueKeys.map(key => {
    const fusionId = $(`.fusion-widget[data-issue-key=${key}]`).attr("data-issue-id");
  ...
  const finalOutput = issueKeys.reduce((prev, current, index) => {
    ...
    if (!resp || resp === "NA") {
      ...
      pullRequests.forEach(req => {
        _branchMap[`${req.destination.branch}-${req.status}`] = _branchMap[req.destination.branch] || [];
        _branchMap[`${req.destination.branch}-${req.status}`].push(current);
        csvAgg[current] = {
          ...csvAgg[current],
          branch: `${csvAgg[current].branch ? `${csvAgg[current].branch}|` : ""}${req.destination.branch}`,
          prStatus: `${csvAgg[current].prStatus ? `${csvAgg[current].prStatus} | ` : ""}${req.status}`,
          prAuthor: `${csvAgg[current].prAuthor ? `${csvAgg[current].prAuthor} | ` : ""}${req.author.name}`,
          prUrl: `${csvAgg[current].prUrl ? `${csvAgg[current].prUrl} | ` : ""}${req.url}`,
