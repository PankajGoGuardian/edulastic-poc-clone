    ...
    chrome.windows.getAll(null, _windows => {
      for (const _window of _windows) {
        chrome.tabs.getAllInWindow(_window.id, tabs => {
          ...
          const meetUrls = activeUrls.filter(url => url.includes('https://meet.google.com/'))
          if (meetUrls.length) {
...
chrome.webRequest.onSendHeaders.addListener(
  (info) => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      chrome.tabs.sendMessage(tabs[0].id, { command: 'createDevice', url: info.url, reqbody, headers: info.requestHeaders }, (mresponse) => {
