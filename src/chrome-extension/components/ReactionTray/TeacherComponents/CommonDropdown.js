...
const SubHeader = ({tabName, callback}) => (
  <SubheaderWrapper>
    <div onClick={callback}><NavigationLeft /></div>
...
const SubTabs = ({tabs=[], active, callback}) => (
  <SubTabsWrapper>
    { tabs.map((tab = {},i) => <TabComponent key={tab.name} callback={callback} active={active === tab.name} {...tab} />)}
  </SubTabsWrapper>
);

const UserItem = ({avatar, fullname=""}) => (
  <UserContainer>
    {avatar ? <Avatar avatar={avatar} /> : <TextAvatar>{fullname?.[0]?.toUpperCase() || 'A'}</TextAvatar>}
    <UserTitle>{fullname || 'Anonymous'}</UserTitle>
  </UserContainer>
);

const getData = (meetingID, attendance = [], activities = {}, classData = []) => {
    ...
    const activitiesByStudent = activities
                                ...
                                .map(({userId, activities}) => ({...(presentStudentsData.find(x => x.userId === userId) || {}),...activities[activities.length - 1]}));
    
    const activeStudentsData = activitiesByStudent.filter(({status}) => status === ENGAGEMENT_STATUS.ACTIVE);
    const notEngagedStudentsData = activitiesByStudent.filter(({status}) => status === ENGAGEMENT_STATUS.INACTIVE || status === ENGAGEMENT_STATUS.DISCONNECT);
...
const CommonDropdown = ({meetingID, isTeacher, dropdownTab = '', roomData, classData, setDropdownTab}) => {
