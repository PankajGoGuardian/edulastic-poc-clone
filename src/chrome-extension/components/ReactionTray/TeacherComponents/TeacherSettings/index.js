...
const TeacherSettings = ({notifications, muteAll, engagementTracking, updateSettings}) => {
  
  const handleMuteAll = (value) => {
    chrome.runtime.sendMessage('eadjoeopijphkogdmabgffpiiebjdgoo',{type: "MUTE_ALL"}, (response) => {
  ...
  return (
    <Container>
      <SettingsRow>
        ...
        <ToggleSwitch checked={notifications} onChange={value => updateSettings({ 'notifications': value})} />
      </SettingsRow>
      ...
      <SettingsRow>
        ...
        <ToggleSwitch checked={engagementTracking} onChange={value => updateSettings({'engagementTracking': value})} />
      </SettingsRow>
    </Container>
    );}
