...
import { setDropdownTabAction, setQuestionItemsAction } from '../../reducers/ducks/edulastic';
...
const StudentTray = ({ authToken, isUserLoaded, activeToneId = 0, dropdownTab, isReactionsVisible, setDropdownTab, setReactionsDropdown }) => (
  <div style={{ display: 'flex', background: 'white' }}>
    {
      authToken ?
        (isUserLoaded && <>
          ...
          <TrayActionButton active={dropdownTab === 'ask'} icon={<IconChat />} callback={() => setDropdownTab("ask")} />
        ...
        (<LoginBtn style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <a href="http://edulastic-poc.snapwiz.net/login#login" target="_blank">Log In</a>
        </LoginBtn>)
    }
  </div>
);
...
const getCurrentEvaluation = async (userAnswer, item) => {
  ...
  const evaluation = await evaluateItem(userAnswer, questions, item.itemLevelScoring, item.maxScore);
  return evaluation;
};
...
const ReactionTray = ({
  ...
  useEffect(() => {
    if (meetingID && userId) {
      MeetFirebase.db?.collection("MeetingUserQuestions").doc(`${meetingID}-${userId}`).onSnapshot((doc) => {
        const { questions = [], evaluations = [] } = doc.data() || {};
        const attemptedItemIds = evaluations.map(({ itemId }) => itemId);
        const pendingItems = questions.filter(({ _id }) => !attemptedItemIds.includes(_id));
  ...
  const handler = async (item, skipped) => {
    if (meetingID && userId) {
      ...
      const evaluation = skipped ? {} : await getCurrentEvaluation(userAnswer, item);
      ...
      const docRef = await MeetFirebase.db.collection('MeetingUserQuestions').doc(`${meetingID}-${userId}`);

      docRef.get().then( (doc) => {
          if (doc.exists) {
              docRef.update({evaluations: firebase.firestore.FieldValue.arrayUnion(_evaluation)})
          }else{
  ...
  return (
    <WrapperContainer>
      <MainTray>
        ...
        {
          userRole &&
            isTeacher ? <TeacherTray {...userProps} /> : <StudentTray authToken={authToken} isUserLoaded={isUserLoaded} {...userProps} />
        }
      </MainTray>
