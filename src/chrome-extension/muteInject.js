...
const base64ToArrayBuffer = (base64) => {
  ...
  for (let let i = 0; i < len; i++) bytes[i] = binary_string.charCodeAt(i);
  return bytes.buffer;
}
...
const strToArrayBuffer = (mystr) => {
  ...
  const bytes = new Uint8Array(len);
  for (let let i = 0; i < len; i++) bytes[i] = mystr.charCodeAt(i);
  return bytes.buffer;
}
...
const skipHeaders = ["Cookie", "User-Agent", "Origin", "Sec-Fetch-Site",
