...
const ActivateDeactivateUser = props => {
  ...
  const updateUser = async () => {
    ...
    try {
      ...
      notification({
        type: "success",
        msg: `User has been ${activate ? "activated" : "deactivated"} successfully`
      });
      clearUserData();
    } catch (error) {
          ...
          {userData.map((data, index) => (
            <Radio key={index} value={index}>
              ...
              <Row>
                ...
                <ValueSpan>{data[openIdMap[data?.openIdProvider?.toLowerCase()]] || "-"}</ValueSpan>
              </Row>
              {data.subscription && (
                <>
                  <Row>
                    ...
                    <ValueSpan>
                      {(
                        data.subscription.subType === "premium" || data.subscription.subType === "enterprise"
                      ).toString()}
                    </ValueSpan>
                  </Row>
                  <Row>
                    <HeadingSpan>Subscription Start Date (YYYY-MM-DD):</HeadingSpan>
                    <ValueSpan>
                      {data.subscription?.subStartDate
                        ? moment(data.subscription.subStartDate).format("YYYY-MM-DD")
                        : "-"}
                    </ValueSpan>
                  </Row>
                  <Row>
                    <HeadingSpan>Subscription End Date (YYYY-MM-DD):</HeadingSpan>
                    <ValueSpan>
                      {data.subscription?.subEndDate ? moment(data.subscription.subEndDate).format("YYYY-MM-DD") : "-"}
                    </ValueSpan>
                  </Row>
                  <Row>
