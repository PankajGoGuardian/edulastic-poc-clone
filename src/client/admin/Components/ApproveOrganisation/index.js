...
const ApproveOrganisation = (props) => {
  ...
  const updateDistrictData = async (isApprove) => {
    ...
    try {
      ...
      notification({
        type: 'success',
        msg: `All schools under this district has been ${
    ...
    <>
      <SecondDiv>
        {!isSchool && (
          <Row>
            <WarningText>
              All the schools in the district will get Approved/Disapproved
            </WarningText>
          </Row>
        )}
        {isSchool && (
        ...
        <Row>
          ...
          <ValueSpan>
            {subDetails?.subType === 'premium' ||
            subDetails?.subType === 'enterprise'
              ? 'Premium'
              : 'Free'}
          </ValueSpan>
        </Row>
      ...
      <ThirdDiv>
        {isSchool ? (
          ...
          <LeftButtonsContainer>
            <EduButton
              onClick={() => updateDistrictData(false)}
              disabled={loading}
              isGhost
            >
              Unapprove
            ...
            <EduButton
              onClick={() => updateDistrictData(true)}
              disabled={loading}
            >
              Approve
            </EduButton>
