...
export function AnswerBox({ btnStyle, index, numeration, centerText, label }) {
  const { scrollHeight: contentHeight, scrollWidth: contentWidth } = measureTextWithImage({ text: label });
...
AnswerBox.propTypes = {
  ...
  numeration: PropTypes.oneOfType([PropTypes.number, PropTypes.string]).isRequired,
