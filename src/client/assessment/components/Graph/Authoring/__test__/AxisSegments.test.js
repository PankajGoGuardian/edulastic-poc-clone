...
describe('<AxisSegments />', () => {
  const component = mount(
    <AxisSegments store={store} graphData={{ stimulus: 'test' }} />
  )
  ...
  it('give external link', () => {
    const input = component.find("input[type='text']").first()
    input.getDOMNode().nodeValue =
      '<a href="alvelenght?77" target="_blank">alvelenght?</a>'
  ...
  it('apply formating', () => {
    const input = component.find("input[type='text']").first()
    input.getDOMNode().nodeValue =
      '<ol><li>Which color has the smallest?</li></ol>'
    input.simulate('click')
  })
