...
const onHandler = (board) => {
  ...
  const newAxis = board.$board.create(
    'axis',
    [
      [isVertical ? calcY : x1, isVertical ? x1 : calcY],
      [isVertical ? calcY : x2, isVertical ? x2 : calcY],
    ],
    {
      ...Colors.numberline,
      straightFirst: false,
  ...
  newAxis.point1.coords.setCoordinates(
    JXG.COORDS_BY_SCREEN,
    newAxis.point1.coords.scrCoords
  )
  newAxis.point2.coords.setCoordinates(
    JXG.COORDS_BY_SCREEN,
    newAxis.point2.coords.scrCoords
  )
  ...
  let labels = ticks.map((t) => {
    ...
    if (
      t === xMin ||
      t === xMax ||
      tickArr.some((specTick) => specTick === t)
    ) {
      let res = null
      if (Number.isInteger(t)) {
        res = t
