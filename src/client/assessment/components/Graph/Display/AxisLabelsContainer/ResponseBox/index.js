...
class ResponseBox extends Component {
  ...
  handleDragStart = (i) => () => {
    window.addEventListener('touchstart', this.preventPageScroll, {
      passive: false,
    })
  ...
  render() {
    ...
    const markCountInLine = Math.floor(
      (optionsContWidth - separationDistanceX) /
        (markWidth + separationDistanceX)
    )
            ...
            <StyledRnd
              ...
              position={{
                x:
                  separationDistanceX +
                  (i % markCountInLine) *
                    (this.dragItemWidth + separationDistanceX),
                y:
                  Math.floor(i / markCountInLine) *
                  (markHeight + separationDistanceY),
              }}
              size={{ width: this.dragItemWidth, height: markHeight }}
              onDragStart={this.handleDragStart(i)}
              onDragStop={(evt, d) =>
                this.handleDragDropValuePosition(
                  d,
                  value,
                  optionsContWidth,
                  height
