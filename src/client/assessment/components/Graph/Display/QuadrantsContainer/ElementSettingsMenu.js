...
export const ElementSettingsMenu = ({
  element,
  handleClose,
  ...
  const [labelIsVisible, handleLabelVisibility] = useState(
    element.labelIsVisible
  )
  const [pointIsVisible, handlePointVisibility] = useState(
    element.pointIsVisible
  )
      ...
      >
        ...
        >
          <Subtitle style={{ margin: 0 }}>
            {utils.capitalizeFirstLetter(element.type)} settings
          </Subtitle>
          <IconClose
        ...
        {showColorPicker && (
          <div style={{ display: 'flex', marginBottom: '20px' }}>
            <ColorPicker
              onChange={(value) => handleColorChange(value.color)}
              animation="slide-up"
              color={elementColor}
            />
