...
class Settings extends Component {
  ...
  handleInputChange = (name, e) => {
    ...
    if (
      ['xMax', 'xMin', 'yMax', 'yMin', 'xDistance', 'yDistance'].includes(
        name
      ) &&
      !value.length
    ) {
      ...
      <Container>
        <Row style={{ width: 0 }}>
          <Label style={{ marginRight: '15px' }}>
            {t('component.graphing.settingsPopup.arrows')}
          </Label>
          <IconWrapper
            selected={this.allArrowsAreVisible()}
            onClick={() => this.handleAllArrowsClick()}
          >
            <IconAllArrows />
          </IconWrapper>
          <IconWrapper
            selected={this.maxArrowsAreVisible()}
            onClick={() => this.handleMaxArrowsClick()}
          >
            <IconMaxArrows />
          </IconWrapper>
        </Row>
