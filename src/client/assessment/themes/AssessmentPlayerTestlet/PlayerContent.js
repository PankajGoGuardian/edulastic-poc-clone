...
const PlayerContent = ({
  ...
  */
  ...
  const findItemIdMap = (scoringId) =>
    find(testletConfig.mapping, ({ testletItemId }) =>
      testletItemId.includes(scoringId)
    )
  ...
  const findTestletValue = (testletId) => {
    ...
    for (const key in testletResponse) {
      if (
        Object.prototype.hasOwnProperty.call(testletResponse, key) &&
        isObject(testletResponse[key])
      ) {
      ...
      if (Object.prototype.hasOwnProperty.call(currentPageIds, scoringId)) {
        ...
        if (cQuestionType === questionType.EXPRESSION_MULTIPART) {
          ...
          Object.keys(responseIds).map((key) => {
            ...
            responseIds[key].map((res) => {
              ...
              if (itemIds) {
                ...
                if (
                  itemIds.type === responseType.dropdown ||
                  itemIds.type === responseType.radio
                ) {
          ...
          data = eduItemResponses.map((eduRes) => {
            const { responseId } =
              find(currentItem.responses, ({ uuid }) => uuid === eduRes.id) ||
              {}
            const testletValue = findTestletValue(responseId)
          ...
          currentItem.responses.map(({ responseId }) => {
            const testletValue = findTestletValue(responseId)
            if (isArray(testletValue)) {
              const selections = testletValue.map((value) =>
          ...
          currentItem.responses.map(
            ({ responseId, elementType, points, labels }) => {
