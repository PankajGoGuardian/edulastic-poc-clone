...
const EditClassification = ({
  ...
  const handleGroupAdd = () => {
    setQuestionData(
      produce(item, (draft) => {
        draft.possibleResponseGroups.push({
          id: uuid(),
          title: '',
          responses: [],
        })
  ...
  const handleGroupRemove = (index) => () => {
    setQuestionData(
      produce(item, (draft) => {
        const responses = draft.possibleResponseGroups[index].responses.flatMap(
          (resp) => resp.id
        )
        responses.forEach((responseID) => {
          ...
          validResponseKeys.forEach((key) => {
            validResponse[key] = validResponse[key].filter(
              (respID) => respID !== responseID
            )
          })
    ...
    setQuestionData(
      produce(item, (draft) => {
        ...
        if (response) {
          ...
          draft.possibleResponseGroups[groupIndex].responses.splice(
            respIndex,
            1
          )
          updateVariables(draft)
      ...
      produce(item, (draft) => {
        switch (action) {
          ...
          case actions.REMOVE:
            ...
            Object.values(
              get(draft, `validation.validResponse.value`, [])
            ).forEach((value = []) => {
        ...
        >
          <GroupPossibleResponses
            ...
            onAdd={
              item.groupPossibleResponses
                ? handleGroupAdd
                : handleMainPossible(actions.ADD)
            }
            onSortEnd={
              item.groupPossibleResponses
                ? handleGroupSortEnd
            ...
            onChange={
              item.groupPossibleResponses
                ? handleGroupChange
                : handleChangePossible()
            }
            onRemoveInner={onRemoveInner}
            onRemove={
              item.groupPossibleResponses
                ? handleGroupRemove
                : handleMainPossible(actions.REMOVE)
            }
            fillSections={fillSections}
            cleanSections={cleanSections}
            t={t}
