...
const AnswerContainer = ({ answer, height, width, isWrapText, fontSize }) => {
  ...
  useEffect(() => {
    ...
    $(parsedHTML).find('img').each(getImageWidth)
  }, [answer])
  ...
  if (imageWidth > imageHeight) {
    imageWidth = Math.round(
      (imageOriginalSize.width * imageHeight) / imageOriginalSize.height
    )
  } else {
    imageHeight = Math.round(
      (imageOriginalSize.height * imageWidth) / imageOriginalSize.width
    )
  }
