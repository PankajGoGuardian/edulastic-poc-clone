...
class ChoicesForDropDown extends Component {
  ...
  onSortEnd = (dropDownId, { oldIndex, newIndex }) => {
    ...
    setQuestionData(
      produce(item, (draft) => {
        draft.options[dropDownId] = arrayMove(
  ...
  remove = (dropDownId, itemIndex) => {
    ...
    setQuestionData(
      produce(item, (draft) => {
        ...
        const validDropDown = cloneDeep(
          draft.validation.validResponse.dropdown.value
        )
  ...
  editOptions = (dropDownId, itemIndex, e) => {
    ...
    setQuestionData(
      produce(item, (draft) => {
        ...
        const drpdwnIndex = findIndex(
          draft.responseIds.dropDowns,
          (drpdwn) => drpdwn.id === dropDownId
        )
        const ind = findIndex(
          draft.responseContainers,
          (cont) => cont.id === dropDownId
        )
        ...
        if (prevAnswerIndex !== -1) {
          ...
          if (prevAnswer && prevAnswer === prevOption) {
            prevDropDownAnswers.splice(prevAnswerIndex, 1, {
              id: dropDownId,
    ...
    setQuestionData(
      produce(item, (draft) => {
        ...
        draft.options[dropDownId].push(
          `${t('component.cloze.dropDown.newChoice')} ${
            draft.options[dropDownId].length + 1
          }`
        )
      })
    )
        ...
        <WidgetWrapper>
          {dropDowns.map((dropdown) => (
            <Question
              ...
              label={`${t('component.math.choicesfordropdown')} ${
                dropdown.index + 1
              }`}
            ...
            >
              <Subtitle
                id={getFormattedAttrId(
                  `${item?.title}-${t('component.math.choicesfordropdown')}`
                )}
              >
                {`${t('component.math.choicesfordropdown')} ${
                  dropdown.index + 1
                }`}
              </Subtitle>
              <SortableList
                ...
                onChange={(itemIndex, e) =>
                  this.editOptions(dropdown.id, itemIndex, e)
                }
              />
