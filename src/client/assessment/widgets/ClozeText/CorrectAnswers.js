  ...
  updateAltAnswersMixMatch = ({ id, tabId, value }) => {
    ...
    const newQuestion = produce(question, (draft) => {
      draft.validation.altResponses = draft.validation.altResponses.map(
        (alt) => {
          if (alt.id === tabId) {
            alt.value = alt.value.filter(
              (resp) => !(resp.id === id && resp.value === value)
            )
          }
          return alt
        }
    ...
    setQuestionData(
      produce(question, (draft) => {
        ...
        draft.validation.altResponses.forEach((altResponse) => {
          const _index = altResponse.value.findIndex(
          ...
          if (_index !== -1 && altResponse.value[_index].value === '') {
            ...
            const resp = draft.responseIds.find(
              (respItem) => respItem.index === index
            )
        ...
        if (!updated) {
          ...
          const validAnswers = draft.validation.validResponse.value.map(
            (answer) => {
