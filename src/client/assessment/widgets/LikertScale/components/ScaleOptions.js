...
const ScaleOptions = ({
  ...
  const handleOptionsSorting = ({ oldIndex, newIndex }) => {
    const updateOptions = arrayMove(options, oldIndex, newIndex)
    if (displayOrder === ASC && newIndex === 0) {
      updateOptions[newIndex].score = 0
    } else if (newIndex === 0) {
      updateOptions[newIndex].score =
        options.length > 1 ? updateOptions[newIndex + 1].score : 0
    } else {
      updateOptions[newIndex].score = updateOptions[newIndex - 1].score
    }
    setQuestionData(
      produce(item, (draft) => {
        draft.options = updateOptions
      })
    )
  }
