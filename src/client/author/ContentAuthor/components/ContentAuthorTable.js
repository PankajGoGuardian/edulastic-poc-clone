...
class ContentAuthorTable extends Component {
        ...
        render: (id, { _source }) => {
          ...
          return (
            <div style={{ whiteSpace: "nowrap" }}>
              {status === 1 ? (
                <StyledTableButton onClick={() => this.onProxyContentAuthor(id)} title={`Act as ${fullName}`}>
                  <GiDominoMask />
                </StyledTableButton>
              ) : null}
            </div>
  ...
  confirmDeactivate = () => {
    ...
    const o = {
      deleteReq: { userIds: selectedAdminsForDeactivate, role: "content-author" },
      listReq: this.getSearchQuery()
    };
              ...
              return (
                <StyledControlDiv key={i}>
                  ...
                  >
                    ...
                    <Option value="username">{t("users.contentAuthor.username")}</Option>
                    <Option value="email">{t("users.contentAuthor.email")}</Option>
                    <Option value="status">{t("users.contentAuthor.status")}</Option>
                  </StyledFilterSelect>
                  <StyledFilterSelect
