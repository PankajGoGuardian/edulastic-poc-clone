...
class CurriculumContainer extends Component {
  ...
  expandAll = () => {
    if (
      ...
      this.setState({
        expandedModules: this.props?.destinationCurriculumSequence?.modules.map(
          (x, index) => index
        ),
      })
    }
  ...
  componentDidUpdate(prevProps) {
    if (
      !prevProps?.destinationCurriculumSequence?.modules &&
      this.props?.destinationCurriculumSequence?.modules
    ) {
      this.expandAll()
    }
  ...
  onDrop = (toModuleIndex, item, afterIndex) => {
    ...
    if (item.fromPlaylistTestsBox) {
      const itemExistsInModule = destinationCurriculumSequence.modules?.[
        toModuleIndex
      ]?.data?.find((x) => x?.contentId === item.id)
      ...
      if (!itemExistsInModule) {
        ...
        notification({
          msg: `Dropped ${
            item.contentType === 'test' ? 'Test' : 'Resource'
  ...
  onOkCollectionsSelectModal = () => {
    const {
      destinationCurriculumSequence,
      approveOrRejectSinglePlaylistRequest,
    } = this.props
  ...
  render() {
    ...
    const curriculumList = Object.keys(curriculumSequences.byId).map(
      (key) => curriculumSequences.byId[key]
    )
    const gradeSubject = {
    ...
    return (
      <>
        <ShareModal
          ...
          isPublished={
            destinationCurriculumSequence.status === statusConstants.PUBLISHED
          }
