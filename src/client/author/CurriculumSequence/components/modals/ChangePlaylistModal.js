...
const sideCardStyle = {font: "11px/15px Open Sans", "font-weight": "600", "letter-spacing": "0.2px", color: lightGreen5};
...
const titleToInitials = (title) => title.split(' ').map(item => {
...
const PlaylistCard = ({
  ...
  return (
    <StyledCard
      key={_id}
      onClick={() => select({_id, title})}
    >
      <Row type="flex" align="middle" style={cardHeaderRowStyle}>
        <Avatar
          size={46}
          style={{"margin-right": "20px", backgroundColor: (active ? lightGreen5 : greyThemeDark1)}}
        >
        ...
        <Tooltip placement="bottomLeft" title={title}>
          <EllipsisContainer
            ...
            color={active ? lightGreen5 : greyThemeDark1}
          >
            {title}
          </EllipsisContainer>
        </Tooltip>
      ...
      <Row type="flex" align="middle" justify="space-between">
        <StyledCol span={12}>
          ...
          <Tooltip placement="bottomLeft" title={`Grade ${grades}`}>
            <EllipsisContainer style={cardTextStyle}>
              Grade {grades}
            </EllipsisContainer>
          </Tooltip>
        </StyledCol>
        ...
        <StyledCol span={24}>
          {playlists.map(playlist => (
            <PlaylistCard
              {...playlist}
              select={onChange}
              active={playlist._id === activePlaylistId}
            />
          ))}
        </StyledCol>
      ) : (
