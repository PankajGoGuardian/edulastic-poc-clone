...
const getFooterComponent = ({ dropPlaylist }) => (
  <FlexContainer width="450px">
    <EduButton
      height="42px"
      data-cy="done-drop-playlist"
      onClick={dropPlaylist}
      inverse
    >
      DONE
    </EduButton>
...
const DropPlaylistModal = (props) => {
  ...
  const [prevAddedClasses, setPrevAddedClasses] = useState(
    droppedAccessData?.classList
  )
  const [prevAddedStudents, setPrevAddedStudents] = useState(
    droppedAccessData?.studentList
  )
  ...
  useEffect(() => {
    if (mode === 'edit') {
      if (searchBy === 'byClass') {
        ...
        setRemovedStudent((prev) =>
          prev?.filter((x) => !studentIds?.includes(x))
        )
      }
    }
  ...
  const handleClassChange = (value, option) => {
    setMode('edit')
    const studentIds = value.flatMap((x) =>
      studentList?.filter((y) => y?.classId === x?.key)?.map((z) => z?.id)
    )
    setAddedStudent((prev) => prev.filter((x) => !studentIds?.includes(x?.id)))
    setAddedClass(
      value.map((x) => ({ id: x?.key, name: x?.label, type: 'class' }))
    )
  }
  ...
  const filterClasses = () => {
    const classIds = addedStudent.flatMap((x) =>
      studentList?.filter((y) => y?.id === x.id)?.map((z) => z?.classId)
    )
  ...
  const dropPlaylist = () => {
    ...
    if (dropPlaylistAction(payload)) {
      notification({
        type: 'success',
        messageKey: 'Playlistdroppedsuccessfully',
      })
      closeModal()
    }
  }
  ...
  const filterNewlyAddedClass = () => {
    ...
    return addedClass
      .filter((c) => !addedClassIds.includes(c.id))
  ...
  const filterNewlyAddedStudents = () => {
    ...
    return addedStudent
      .filter((c) => !addedStudentIds.includes(c.id))
      .map((x) => ({ key: x.id, label: x.name }))
  }

  const addedData = uniqBy(
    [
      ...prevAddedClasses,
      ...filterClasses(),
      ...prevAddedStudents,
  ...
  const addedClassIds = [...prevAddedClasses, ...filterClasses()].map(
    (x) => x?.id
  )
  const addedStudentIds = [...prevAddedStudents, ...addedStudent].map(
    (x) => x?.id
  )
  ...
  const filterStudents = () => {
    ...
    return studentList.filter(
      (x) => ids.includes(x?.classId) && !addedStudentIds.includes(x?.id)
    )
  }
    ...
    >
      <>
        <p style={{ marginBottom: '21px' }}>
          Dropping a playlist will let other teachers/students see the playlist
          and follow along.
        </p>
        <Selector
...
export default connect(
  (state) => ({
    ...
    droppedAccessData:
      state?.curriculumSequence?.dropPlaylistSource?.droppedAccess,
    destinationCurriculumSequence:
