    ...
    form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        if (
          ...
          values.confirmationText.toUpperCase() === 'REMOVE'
        )
          removeSchool()
      }
    })
  ...
  return (
    ...
    >
      <ModalBody data-cy="removeSchoolModal">
        <span>
          You are about to remove the school{' '}
          <strong>{selectedSchool.name}</strong>.
        </span>
        <span>
          All assessment and questions shared with {selectedSchool.name} would
          be moved to private Library.
        </span>
        <span>
          This action can NOT be undone, please type{' '}
          <strong style={{ color: themeColor }}>REMOVE</strong> in the space
          below to proceed.
        </span>
        <FormItem>
