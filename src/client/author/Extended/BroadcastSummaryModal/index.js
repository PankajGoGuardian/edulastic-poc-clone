...
const BroadcastTitle = ({ classList = [], setClassId, value, counts = {} }) => (
  <FlexContainer height="40px" padding="10px 0 0 0" justifyContent="space-evenly">
    ...
    <FlexContainer width="300px" justifyContent="space-evenly">
      <Stats label="CORRECT" count={Object.keys(counts).length ? counts.correct : '-'} />
      <Stats label="INCORRECT" count={Object.keys(counts).length ? counts.incorrect : '-'} />
      <Stats label="NOT ENGAGED" count={Object.keys(counts).length ? counts.notEngaged : '-'} />
    </FlexContainer>
    ...
    <div>
      ...
      >
        {
          classList.map(({ _id, name }) => <Select.Option value={_id}>{name}</Select.Option>)
        }
      </Select>
    </div>
  </FlexContainer>
...
const QuestionNavigation = ({ disable, icon, clickCallback }) => (
  <FlexContainer width="40px" alignItems="center" justifyContent="center">
    {disable && <FontAwesomeIcon icon={icon} aria-hidden="true" style={{ color: "#505050", fontSize: "30px", cursor: "pointer" }} onClick={clickCallback} />}
  </FlexContainer>
);
...
const BroadcastSummaryModal = ({
  ...
  useEffect(() => {
    if (meetingID) {
      MeetFirebase.db.collection("MeetingUserQuestions")
        ...
        .get().then((snapshot) => {
          ...
          questions.forEach(question => {
            ...
            docs.forEach(doc => {
              ...
              const evaluation = evaluations.find(({ itemId }) => itemId === qId);
  ...
  const questions = { ...keyBy(item?.data?.questions, "id"), ...keyBy(item?.data?.resources, "id") };
  ...
  return (
    <Modal
      width="900px"
      visible={visible}
      title={<BroadcastTitle value={classId} setClassId={handleClassChange} classList={classList} counts={counts[item?._id]} />}
      onCancel={closeModal}
      footer={null}
      bodyStyle={bodyStyle}
    >
      <FlexContainer height="" justifyContent='space-between'>
        <QuestionNavigation disable={!isFirstItem} icon={faAngleLeft} clickCallback={() => setCurrentIndex(previousIndex => previousIndex - 1)} />
          ...
          /> : (
            <FlexContainer alignItems='center' height='100%'>
              <Empty
                ...
                description={
                  <span>
                    No Broadcasted Questions to this Class
                  </span>
                  }
              />
            </FlexContainer>
        ...
        <QuestionNavigation disable={!isLastItem} icon={faAngleRight} clickCallback={() => setCurrentIndex(previousIndex => previousIndex + 1)} />
      </FlexContainer>
    </Modal>
  );
};
