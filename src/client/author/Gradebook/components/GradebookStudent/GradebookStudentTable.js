...
const GradebookStudentTable = ({ t, dataSource = [], windowHeight }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, row) => (
        <>
          <div>{row.maxScore ? `${round(row.score || 0, 2)}/${row.maxScore || 1}` : "-"}</div>
          {row.archived.map(ta => (
    ...
    },
    {
      ...
      render: (data, row) => (
        <>
          ...
          {row.archived.map(ta => (
            <div style={{ paddingTop: "5px" }}>{ta.percentScore?.trim() ? ta.percentScore : "-"}</div>
          ))}
        </>
      ),
      sorter: (a, b) => {
    ...
    },
    {
      ...
      render: data =>
        data ? (
          <StyledTag bgColor={statusMap[data].color} textColor={statusMap[data].fgColor}>
            {statusMap[data].name}
          </StyledTag>
        ) : (
          "-"
