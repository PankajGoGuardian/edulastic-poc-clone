...
class ClassEdit extends React.Component {
  ...
  componentDidUpdate(
    { updating, history, selctedClass, location },
    { submitted }
  ) {
    ...
    if (updating && submitted) {
      ...
      history.push(
        (exitToShow ? showPath : exitPath) || `/author/manageClass/${classId}`
      )
    }
  }
  ...
  handleSubmit = (e) => {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        values.startDate =
          moment(startDate).format('x') || selctedClass.startDate
        values.tags = tags.map((t) => allTagsData.find((o) => o._id === t))

        const editedClassValues = { ...values }
    ...
    return (
      <Form onSubmit={this.handleSubmit} style={{ position: 'relative' }}>
        <Header
          classId={classId}
          type={type}
          exitPath={exitToShow ? showPath : exitPath}
        />
        <Spin spinning={updating}>
          ...
          <Container>
            <FormTitle>
              {type === 'custom' ? 'Group' : 'Class'} Details
            </FormTitle>
            <Row gutter={36}>
              <LeftContainer xs={8}>
                <LeftFields
  ...
  connect(
    (state) => {
      ...
      return {
        ...
        classLoaded:
          get(state, 'manageClass.classLoaded') &&
          get(state, 'manageClass.loaded'),
