...
import ItemsTable from "../../../TestPage/components/common/ItemsTable/ItemsTable";
import ItemFilter from "../../../ItemList/components/ItemFilter/ItemFilter";
import { getClearSearchState, filterMenuItems } from "../../../ItemList";
import ModalCreateTestItem from "../../../TestPage/components/ModalCreateTestItem/ModalCreateTestItem";

class AddTests extends PureComponent {
  static propTypes = {
  ...
  componentDidMount() {
    ...
    const { selectedItems, getCurriculums, receiveTests, limit } = this.props;
    this.setState({
      selectedTestItems: selectedItems
    });

    getCurriculums();
    receiveTests(search, 1, limit);
  }
  ...
  handleLabelSearch = e => {
    const { search } = this.state;
    const { limit, receiveTests } = this.props;
    ...
    receiveTests({ ...search, filter }, 1, limit);
  };
  ...
  handlePaginationChange = newPage => {
    const { receiveTests, limit } = this.props;
    const { search } = this.state;
    receiveTests(search, newPage, limit);
  };
...
export default enhance(AddTests);
