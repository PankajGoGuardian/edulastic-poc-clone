...
const GET_CUSTOM_REPORT_STATE_REQUEST_ERROR = "[reports] get custom reports error";

const GET_CUSTOM_REPORT_URL_REQUEST = "[reports] get custom reports url request";
const GET_CUSTOM_REPORT_URL_REQUEST_SUCCESS = "[reports] get custom url reports success";
const GET_CUSTOM_REPORT_URL_REQUEST_ERROR = "[reports] get custom url reports error";

// -----|-----|-----|-----| ACTIONS BEGIN |-----|-----|-----|----- //

export const getCustomReportAction = createAction(GET_CUSTOM_REPORT_STATE_REQUEST);
export const getCustomReportURLAction = createAction(GET_CUSTOM_REPORT_URL_REQUEST);
...
export const getCustomReportURL = createSelector(
  stateSelector,
  state => (state.reportURL ? state.reportURL.url : "")
);

export const getCustomReportName = createSelector(
  stateSelector,
  state => (state.reportURL ? state.reportURL.name : "")
);
...
export function* customReportSaga() {
  yield all([yield takeLatest(GET_CUSTOM_REPORT_STATE_REQUEST, getCustomReportRequest)]);
  yield all([yield takeLatest(GET_CUSTOM_REPORT_URL_REQUEST, getCustomReportURLRequest)]);
}
