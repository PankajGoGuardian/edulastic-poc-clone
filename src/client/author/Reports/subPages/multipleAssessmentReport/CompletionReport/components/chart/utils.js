  ...
  return chartData
    .map((item) => {
      ...
      const notOpenWidth =
        item.notOpen === '0'
          ? 0
          : calculateMinWidth(item.notOpen, item.assigned)
      ...
      const totalWidth =
        ...
        submittedWidth +
        notOpenWidth
      const formattedDate = formatDate(item.assessmentDate)
      const testDate =
        formattedDate === 'Invalid date' ? 'Not Open' : formattedDate
      return {
        testName: item.testName,
        gradedPercentage: ((item.graded / item.assigned) * 100).toFixed(2),
        submittedPercentage: ((item.submitted / item.assigned) * 100).toFixed(
          2
        ),
