...
import { getReportsStandardsPerformanceSummary } from '../../socialEmotionalLearning/ducks'
...
const SEL_STANDARD_SET = ['1530']
const SEL_STANDARD_PROFICIENCY = ['63932d3966309c0008e069bc']

const NonAcademicReportFilters = ({
  isPrinting,
  user,
  history,
  location,
  ...
  const skillInfoOptions = {
    [reportNavType.SOCIAL_EMOTIONAL_LEARNING]: socialEmotionalLearning,
  }
  ...
  if (prevStandardsFilters !== standardsFilters && !isEmpty(standardsFilters)) {
    ...
    if (reportId) {
      ...
      // hardcoded Delaware - Social and Emotional Learning Competencies instead of search.curriculumId
      const urlCurriculum =
        curriculumsList.find((item) => SEL_STANDARD_SET.includes(item.key)) ||
        curriculumsList[0]
      const urlStandardGrade =
        staticDropDownData.allGrades.find(
          (item) => item.key === search.standardGrade
        ) || staticDropDownData.allGrades[0]
      // hardcoded SEL standard proficiency for demo
      const urlStandardProficiency =
        standardProficiencyList.find((item) =>
          SEL_STANDARD_PROFICIENCY.includes(item.key)
              ...
              <MultipleSelect
                ...
                valueToDisplay={
                  selectedDomains.length > 1
                    ? { key: '', title: 'Multiple Competencies' }
                    : selectedDomains
                }
                by={selectedDomains}
                prefix="Competency"
...
const enhance = compose(
  connect(
    (state) => ({
      ...
      socialEmotionalLearning: getReportsStandardsPerformanceSummary(state),
...
export default enhance(NonAcademicReportFilters)
