...
const fillColors = (data, selectedMastery) => {
  ...
  return map(data, (item) =>
    next(item, (draftItem) => {
      draftItem.color = includes(selectedMastery, item.masteryLabel)
        ? item.color
        : '#cccccc'
    })
  )
...
const StudentPerformancePie = ({
  ...
  const percentageScaleInfo = (scaleInfo || []).map((item) => {
    item.percentage = scaleMapKeys.includes(item.masteryLabel)
      ? scaleMap[item.masteryLabel]
      : 0
    return item
  })
