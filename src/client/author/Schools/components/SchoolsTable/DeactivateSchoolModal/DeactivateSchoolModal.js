...
class DeactivateSchoolModal extends React.Component {
  ...
  render() {
    ...
    const schoolName = schoolData.map((row) => (
      <StyledClassName>{row.name}</StyledClassName>
    ))
      ...
      >
        <ModalBody>
          <span>
            ...
            {t('school.components.deactivateschool.schools')}
          </span>
          {schoolName}
          <span>
            {t('common.modalConfirmationText2')}
            <LightGreenSpan>
              {t('school.components.deactivateschool.deactivate')}
            </LightGreenSpan>{' '}
