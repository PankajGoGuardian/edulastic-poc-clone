...
import { CustomModalStyled,
     EduButton,FieldLabel,
     notification,
     TextInputStyled, } from '@edulastic/common'
...
function CartModal({
    ...
    const isMultipleQuantities = Object.keys(quantities).find(x => quantities[x] > 0)
    ...
    const selectedProductIds = Object.keys(quantities).filter(x => quantities[x]);

    return (<CustomModalStyled
        visible={visible}
        ...
        onCancel={closeModal}
    >
        <p style={pStyle}>50 student licenses are included in each teacher license for free. Additional student licenses cost <strong>$2/student</strong></p>

        <p style={pStyle}>There are additional products that can make instructions easier. Subscribe to SparkMath, Spark Reading free trials to see how they can help. </p>
        ...
        />
        {isMultipleQuantities && (<EmailWrapper>
