...
import { QuestionDetails, DetailRow, FlexWrap, StyledSpan } from './styled'
...
const QuestionPreviewDetails = ({
  ...
  const rubricNamesLength = useMemo(() => {
    return allRubricNames.length
  }, [allRubricNames])

  const [currentRubricId, setCurrentRubricId] = useState(null)

  const handleUpdateRubricId = (_id) => {
    setCurrentRubricId(_id)
  }
    ...
    <>
      {currentRubricId && (
        <RubricInfoModal
          ...
          handleUpdateRubricId={handleUpdateRubricId}
        />
      )}
      <QuestionDetails>
        <FlexWrap>
