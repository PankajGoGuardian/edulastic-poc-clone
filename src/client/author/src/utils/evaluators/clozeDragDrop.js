const clozeDragDropEvaluation = ({ userResponse, validation, hasGroupResponses }) => {
  ...
  userResponse.forEach((resp, index) => {
    ...
    if (hasGroupResponses) {
      result[index] =
        (validResponse.value[index].group === resp.group && validResponse.value[index].data === resp.data) ||
        (alternateResponses[index] &&
          alternateResponses[index].group === resp.group &&
          alternateResponses[index].data === resp.data);
    } else {
      result[index] = validResponse.value[index] === resp || alternateResponses[index] === resp;
    }
  });
  return result;
};
