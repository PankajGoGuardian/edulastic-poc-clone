...
export const getAnswersFromVideo = (cv, matSrc) => {
  ...
  if (qrCode) {
    let leftBottomPoint = {
      x: (qrCode.location.bottomLeftCorner.x * qrRectWidth) / 200 + qrRect.x,
      y: (qrCode.location.bottomLeftCorner.y * qrRectWidth) / 200,
    }
    let rightBottomPoint = {
      x: (qrCode.location.bottomRightCorner.x * qrRectWidth) / 200 + qrRect.x,
      y: (qrCode.location.bottomRightCorner.y * qrRectWidth) / 200,
    }
    let leftTopPoint = {
      x: (qrCode.location.topLeftCorner.x * qrRectWidth) / 200 + qrRect.x,
      y: (qrCode.location.topLeftCorner.y * qrRectWidth) / 200,
    }
    let rightTopPoint = {
      x: (qrCode.location.topRightCorner.x * qrRectWidth) / 200 + qrRect.x,
      y: (qrCode.location.topRightCorner.y * qrRectWidth) / 200,
    }
  ...
  for (let i = 0; i < contours.size(); ++i) {
    ...
    if (
      ...
      rect.width < widthOfQR * 4 &&
      rect.height > widthOfQR / 2
    ) {
      rectArray.push(rect)
    }
  ...
  cv.putText(
    matSrc,
    'Place QR code',
    { x: rectStartPoint.x + 20, y: rectStartPoint.y + 25 },
      ...
      if (countOfElementsInTheSameLine === 0) {
        ...
        if (listForCurrentLine.length > 1) {
          ...
          for (let i = 0; i < circlesInCropped.cols; i++) {
            ...
            let color =
              copyForMark.data[
                parseInt(y) * croppedImg.cols * croppedImg.channels() +
                  parseInt(x) * croppedImg.channels()
              ]
            circlesCroppedArray.push({
              center: center,
              radius: radius,
              color: color,
            })
          ...
          circlesCroppedArray.sort(
            (firstItem, secondItem) => firstItem.color - secondItem.color
          )
          let minColorValue = -1
