...
class DomainDetail extends Component {
  ...
  render() {
    ...
    if (summary) {
      ...
      const summaryStandards = summary.standards
        .filter(std => reportData[std.id]);
      score = summaryStandards
        .reduce((acc, std) => acc + reportData[std.id].score, 0);
      maxScore = summaryStandards
        .reduce((acc, std) => acc + reportData[std.id].maxScore, 0);
      // or
    ...
    return (
      <S.AssignmentContentWrap>
        <S.Title onClick={this.handlerTable}>
          ...
          <S.StyledScoreProgress percent={Math.round(score / maxScore * 100)} />
